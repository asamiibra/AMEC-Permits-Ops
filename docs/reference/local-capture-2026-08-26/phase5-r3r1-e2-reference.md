# CRITICAL R3R1-E2 SUPERSESSION — LOCAL PRECOMMIT VS REMOTE NATIVE RUNTIME BOUNDARY
## ProposalOps / AMEC Phase5 R3R1 — remote-native SQL Server / browser execution correction
**Date:** 2026-08-25

This E2 block is a narrowly superseding correction prepended to the complete
R3R1E1 authorization below.

The complete R3R1E1 authorization, including its exact-byte Appendix-B
governing-source bundle, remains in force except where this E2 block explicitly
supersedes a host-local runtime requirement.

---

# E2-0. WHY THIS SUPERSESSION EXISTS

The immediately preceding R3R1E1 attempt correctly passed the exact-byte E01
bootstrap and completed the deterministic/source/unit/frontend precommit work,
but stopped before commit because its current execution host has no native SQL
Server runtime:

```text
RESULT=STOP_PHASE5_R3R1_APPLICATION_PRECOMMIT_FAILED

LOCAL_DOCKER_PRESENT=false
LOCAL_SQLCMD_PRESENT=false
LOCAL_ISQL_PRESENT=false

NATIVE_SQLSERVER_PRECOMMIT=
ENVIRONMENT_BLOCKED_LOCAL_ONLY

NO_APPLICATION_COMMIT_CREATED=true
NO_WORKFLOW_COMMIT_CREATED=true
NO_REMOTE_R3R1_BRANCH_PUSHED=true
```

The stop was caused by an authorization-boundary error in R3R1E1: the local
precommit matrix required `backend/tests/test_phase5_sqlserver_runtime.py` to
execute against native SQL Server even though the successful R3 GitHub Actions
harness is the authorized native x64 SQL Server environment.

That host-dependent native runtime must be proven **remotely**, not fabricated
locally and not replaced with SQLite.

This E2 block corrects that boundary.

---

# E2-1. EXACT LIVE REPOSITORY AUTHORITY — REPROVE FIRST

Before mutation or commit, fetch origin and require:

```text
R3_APPLICATION_BRANCH=
phase5-classifier-shadow-validation-r3-v1

R3_APPLICATION_SHA=
46c91cb74f8e686d99a82809ccc73553c7c781d1

R3_APPLICATION_PARENT=
ca60459f2103cbf30e52e05c59b8af6d7714be12

R3_APPLICATION_TREE=
0aacf61b86c3dbfbbe877c001a67b2f94eba8ef1

R3_VALIDATION_BRANCH=
phase5-classifier-shadow-validation-ci-r3-v1

R3_VALIDATION_SHA=
a5e6b9425ead924e35bb55cafc065251de0cb021

R3_VALIDATION_PARENT=
46c91cb74f8e686d99a82809ccc73553c7c781d1
...
Require remote R3R1 target branches still absent:

```text
phase5-classifier-shadow-validation-r3r1-v1=ABSENT
phase5-classifier-shadow-validation-ci-r3r1-v1=ABSENT
```

If either remote R3R1 branch already exists unexpectedly:

```text
RESULT=STOP_PHASE5_R3R1E2_LIVE_IDENTITY_MISMATCH
```

STOP.

Do not delete or rewrite any remote branch.

---

# E2-2. READ-ONLY SIBLING REPAIR REFERENCE — NEVER THE BASE

A sibling repair branch currently exists and may be used only as a read-only
source/reference:

```text
READ_ONLY_REFERENCE_BRANCH=
phase5-classifier-shadow-validation-r3r2-v1

READ_ONLY_REFERENCE_SHA=
9b57f0dd4937790b3a737357e72407b4afef8c73

READ_ONLY_REFERENCE_PARENT=
46c91cb74f8e686d99a82809ccc73553c7c781d1

REFERENCE_ROLE=
READ_ONLY_REPAIR_AND_GOVERNING_SOURCE_REFERENCE

REFERENCE_NOT_BASE=true
REFERENCE_NOT_ACCEPTED=true
REFERENCE_CHERRY_PICK_FORBIDDEN=true
```

It is one sibling commit above R3 and contains useful evidence-truth repair
bytes plus the exact four governing-source files.

Do **not**:
- base R3R1 on `9b57f0dd...`;
- cherry-pick `9b57f0dd...`;
- call it accepted;
- inherit its bytes without inspecting them against this authorization.

The R3R1 application parent remains exactly:

```text
46c91cb74f8e686d99a82809ccc73553c7c781d1
```

If the prior local R3R1 worktree still exists, reuse it only when:
- HEAD is exact R3 `46c91cb...`;
- no commit was created;
- all modified/untracked repository paths are within the R3R1 allowlist;
- generated `artifacts/**` remain unstaged.

If that temporary worktree is gone, reconstruct a fresh R3R1 worktree from
exact R3. The sibling may be read for repair reference only; materialize only
authorized paths and independently apply the R3R1/E1/E2 corrections.

Never alter the user's unrelated main checkout/worktree.

---

# E2-3. E01 EXACT-BYTE BOOTSTRAP REMAINS MANDATORY AND CLOSED ONLY BY HASH

The E01 correction from R3R1E1 remains fully authoritative.

Reconstruct/materialize the four governing source files from Appendix-B
base64 bytes in this prompt and require exact:
...
GOVERNING_SOURCE_BYTE_COUNT_MISMATCH_COUNT=0
```

Do not trust the sibling branch manifest without recomputing the four hashes.

---

# E2-4. GLOBAL CONFLICT RULE — WHAT E2 SUPERSEDES

Where any later embedded R3R1E1/R3 text conflicts with this E2 block, E2 wins.

E2 supersedes **only**:

1. every local/precommit command that requires
   `backend/tests/test_phase5_sqlserver_runtime.py` to execute without a real
   native MSSQL endpoint;

2. every local/precommit requirement that treats unavailable local
   Docker/SQL Server as an application failure;

3. mandatory local execution of nonexistent files:
   ```text
   backend/tests/test_phase5_integration.py
   backend/tests/test_phase5_shadow_runtime.py
   ```

4. any statement that requires fresh native SQL Server or real browser runtime
   evidence **before** the R3R1 application commit;

5. any acceptance condition that could promote local SQLite/unit/source results
   into Phase5 native-SQL-Server or browser runtime proof.

E2 does **not** supersede:
- the application mutation allowlist;
- the frozen classifier/Phase4/model/migration/dependency/frontend boundaries;
- E01 exact-byte source requirements;
- C06/C08/C19C/C07/C20 repair requirements;
- the 300-row evidence mapping requirement;
- the false-accept matrix;
- source preflight V4;
- the whole-surface no-loop audit;
- one-application-commit / one-workflow-commit discipline;
- the full fresh GitHub-hosted native SQL Server runtime;
- the real browser runtime;
- sanitizer/finalizer/handoff requirements;
- safety boundaries;
- independent-acceptance separation.

---

# E2-5. LOCAL ENVIRONMENT CLASSIFICATION IS NOT A PRODUCT RESULT

At local precommit, detect whether a usable native SQL Server endpoint exists.
...
This state is neither PASS nor application FAIL.

Never run the native SQL Server test module against SQLite and interpret the
expected MSSQL assertion failure as a product defect.

Never install or troubleshoot local Docker merely to satisfy Phase5 R3R1.

Do not fabricate:
- SQL Server test PASS;
- browser runtime PASS;
- SQL Server evidence files;
- browser evidence files.

Remote GitHub Actions is the required native runtime authority.

---

# E2-6. EXACT LOCAL PRECOMMIT MATRIX — SUPERSEDES CONFLICTING LOCAL MATRICES

## E2-6.1 Native SQL Server module: collection/contract only
...
Also source-audit the module and require:
- it refuses non-`mssql+pyodbc` `DATABASE_URL`;
- the 16 canonical gate IDs remain exactly present;
- no skip/xfail was added to hide runtime requirements.

Do **not** execute those 16 native tests locally unless a real native MSSQL
endpoint is already independently available. Local native execution is
optional diagnostic evidence only and is never required for the app commit.

## E2-6.2 Existing concrete backend tests
...
Then full backend regression excluding only the separately remote-required
native SQL Server test module:

```bash
pytest -q --ignore=backend/tests/test_phase5_sqlserver_runtime.py
```

Require zero actual failures.

The expected current full-backend shape may change as R3R1 tests are added;
do not hard-code `488` as an acceptance count. Require the observed count,
zero failures, and zero unexpected collection errors.

## E2-6.3 Two obsolete/nonexistent test references

At exact R3, these paths do not exist:
...
R3R1 does not authorize creating them.

Record:

```text
PHASE5_INTEGRATION_TEST_FILE_STATE=
NOT_APPLICABLE_NONEXISTENT_SOURCE

PHASE5_SHADOW_RUNTIME_TEST_FILE_STATE=
NOT_APPLICABLE_NONEXISTENT_SOURCE
```
...
- not PASS;
- not FAIL;
- not a scope-expansion reason.

The required integration/shadow runtime evidence is produced by the actual
existing Phase4 integration test, `scripts/phase5/shadow_replay.py`, and the
remote runtime workflow.

## E2-6.4 Deterministic/source tooling
...
Require:
- source preflight V4 PASS;
- reproducibility PASS;
- corpus coverage PASS;
- governing-source hashes exact;
- no definite whole-surface blocker;
- no unclassified source/evidence producer;
- no diff-check error.

## E2-6.5 Frontend local source/build preflight

Use the exact tracked R3/R3R1 frontend tree.

If an isolated worktree is missing tracked files, restore **only missing
tracked bytes from the exact candidate index**. Do not modify the candidate
index or stage those restorations as changes.
...
Require zero actual unit/type/build/collection failures.

`--list` proves browser-test discovery only.

Do not claim real browser runtime proof locally.

---

# E2-7. PRECOMMIT FALSE-ACCEPT MATRIX — RUNTIME MUST REMAIN NOT_EXECUTED

Before application commit, all runtime-required evidence producers in the
local dry-run fixture must remain:
...
No local environment blocker may be converted into synthetic runtime PASS.

---

# E2-8. FINALIZER CIRCULARITY — EXPLICIT NO-SELF INVARIANT

The previous local R3R1E1 work correctly identified a circularity in the
read-only sibling repair.

The central category policy must **not** require the `finalizer` producer as a
prerequisite for the `FINALIZER` acceptance category before the finalizer has
run.

Require:

```text
FINALIZER_CATEGORY_REQUIRES_FINALIZER_PRODUCER=false
FINALIZER_SELF_PREREQUISITE_COUNT=0

FINALIZER_CATEGORY_REQUIRED_PRE_FINALIZER_PRODUCERS=
freeze-reproducibility
```

The actual finalizer remains downstream and must independently:
- validate all required producer metadata;
- require exact candidate SHA;
- require exact validation SHA;
- require exact run ID;
- write its own triple-bound result/meta/summary;
- fail closed on missing/failed/NOT_EXECUTED runtime evidence.

Do not preseed a finalizer PASS.
...
# E2-9. WHOLE-SURFACE NO-LOOP AUDIT V5 — BEFORE APPLICATION COMMIT

Extend the R3R1 whole-surface audit.
...
the planned R3R1 validation workflow
```

Mechanically require:

```text
PHASE5_R3R1_WHOLE_SURFACE_AUDIT_VERSION=5

PHASE5_R3R1_WHOLE_SURFACE_DEFINITE_BLOCKER_COUNT=0

PHASE5_RUNTIME_ONLY_LOCAL_EXECUTION_REQUIREMENT_COUNT=0

PHASE5_NONEXISTENT_REQUIRED_TEST_PATH_COUNT=0

PHASE5_FINALIZER_SELF_PREREQUISITE_COUNT=0

PHASE5_WORKFLOW_UNKNOWN_CLI_ARGUMENT_COUNT=0
PHASE5_WORKFLOW_MISSING_EXECUTABLE_PATH_COUNT=0
PHASE5_WORKFLOW_UNKNOWN_PRODUCER_COUNT=0
PHASE5_WORKFLOW_REQUIRED_EVIDENCE_NAME_MISMATCH_COUNT=0

PHASE5_WORKFLOW_SQLSERVER_TARGETED_DATABASE_URL_IS_MSSQL=true
PHASE5_WORKFLOW_SQLSERVER_RUNTIME_TEST_FILE_PRESENT=true
PHASE5_WORKFLOW_SQLSERVER_RUNTIME_TEST_COLLECTED_GATE_COUNT=16

PHASE5_WORKFLOW_FULL_BACKEND_EXCLUDES_NATIVE_SQLSERVER_MODULE=true

PHASE5_WORKFLOW_REAL_BROWSER_USES_SQLSERVER_BACKED_API=true

PHASE5_WORKFLOW_ONLY_SANITIZED_EVIDENCE_UPLOADED=true

PHASE5_GOVERNING_SOURCE_HASH_MISMATCH_COUNT=0

PHASE5_CLASSIFIER_BEHAVIOR_PATH_CHANGE_COUNT=0
PHASE5_PHASE4_PATH_CHANGE_COUNT=0
PHASE5_MIGRATION_MODEL_CHANGE_COUNT=0
PHASE5_DEPENDENCY_CHANGE_COUNT=0
PHASE5_FRONTEND_CHANGE_COUNT=0
```

The audit must inspect executable parser definitions and workflow command
arguments, not merely search for expected strings.

If a new definite blocker requires a non-allowlisted application path:

```text
RESULT=STOP_PHASE5_R3R1E2_ADDITIONAL_DEFINITE_BLOCKER_FOUND
BLOCKER=<exact>
REQUIRED_PATH=<exact>
```
...
# E2-10. APPLICATION COMMIT AUTHORIZATION AFTER DETERMINISTIC PRECOMMIT PASS

If and only if:
- E01 exact bytes PASS;
- R3R1 repair ledger all CLOSED_WITH_EXECUTABLE_REGRESSION;
- local deterministic/source/unit/integration/frontend source/build matrix PASS;
- SQL Server test module collects exactly 16;
- local runtime-required acceptance credit remains zero;
- false-accept matrix is zero;
- whole-surface audit V5 has zero definite blockers;
- diff is within the original R3R1 allowlist;

then the absence of native local SQL Server is **not** a blocker to the
application commit.

Create exactly one R3R1 application commit as already authorized:
...
No second application commit is authorized.

---

# E2-11. REMOTE NATIVE RUNTIME IS MANDATORY AND AUTHORITATIVE

After the one R3R1 application commit, create exactly one validation-workflow
child commit on:
...
Use successful R3 validation workflow:
...
as runtime structural authority, with the R3R1 evidence-truth/finalizer/source
repairs applied.

The R3R1 remote workflow must use:

```text
runs-on=ubuntu-22.04
architecture=x86_64
Docker=REQUIRED
ODBC_DRIVER_18=REQUIRED
SQLSERVER_2022_DEVELOPER=REQUIRED
PLAYWRIGHT_CHROMIUM=REQUIRED
```

Use fresh databases and fresh runtime evidence.

The native SQL Server producer must execute:
...
The full backend producer must separately run:

```bash
pytest -q --ignore=backend/tests/test_phase5_sqlserver_runtime.py
```

No duplication or substitution.

---

# E2-12. REMOTE REAL BROWSER PROOF — SAME SQL SERVER-BACKED STACK

The remote workflow must start the actual backend against fresh native SQL
Server and the real frontend, then run the required Playwright Phase5 spec.
...
No local `--list` result can satisfy these gates.

---

# E2-13. REMOTE EVIDENCE-TRUTH CLOSURE REMAINS EXACT

After actual runtime producers exist, require all original R3R1 closure gates,
including:
...
Sanitize only after successful finalization.

Upload only sanitized evidence.

---

# E2-14. SUCCESS DOES NOT MEAN AZURE SQL SERVICE VERIFIED

A successful GitHub-hosted SQL Server 2022 run may prove the Phase5 native
SQL-Server compatibility/runtime lane and real browser evidence for this exact
candidate.
...
# E2-15. FAILURE POLICY — ONE RUN, NO PATCH LOOP

If deterministic/source/unit precommit fails for a real source reason:

```text
RESULT=STOP_PHASE5_R3R1E2_APPLICATION_PRECOMMIT_FAILED
...
Do not classify absent local Docker/SQL Server as this result when all local
deterministic gates are green.

If remote runtime fails:

```text
RESULT=STOP_PHASE5_R3R1E2_REMOTE_VALIDATION_FAILED
FIRST_FAILED_GATE=<exact>
...
Do not automatically create another Phase5 application/workflow repair.
Return the exact first failure.

---

# E2-16. FINAL SUCCESS RETURN

Only if:
- exact-byte E01 remains PASS;
- one R3R1 application commit exists directly on `46c91cb...`;
- one workflow-only validation child exists;
- the fresh remote native SQL Server 16/16 producer passes;
- the fresh real-browser 10/10 producer passes;
- all regressions/builds pass;
- 300/300 acceptance mapping passes;
- validator/finalizer false-accept count is zero;
- finalizer triple binding passes;
- sanitized artifact verification passes;
- independent artifact recheck required by the original R3R1 authorization
  passes;
...
```text
RESULT=
PHASE5_FINAL_R3R1_CANDIDATE_READY_FOR_REBOUND_INDEPENDENT_ACCEPTANCE
...
PHASE5_R3R1_VALIDATION_SHA=<exact>
PHASE5_R3R1_VALIDATION_PARENT=<PHASE5_R3R1_CANDIDATE_SHA>
...
NEXT=
REBOUND_INDEPENDENT_PHASE5_ACCEPTANCE_REVIEW
```
...
# E2-17. IMPORTANT EXECUTION NOTE

The complete R3R1E1 authorization follows below byte-for-byte.

Its Appendix-B exact-source bundle remains usable.

Do not stop merely because an older embedded local-precommit section still
contains a native SQL Server command. E2-4 and E2-6 explicitly supersede that
host-local requirement.

All remote native-SQL-Server/browser requirements remain mandatory.
