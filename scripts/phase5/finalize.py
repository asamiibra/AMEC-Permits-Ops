...
def _errors(contracts_dir: Path, acceptance_path: Path, evidence_dir: Path, expected: str, validation_result_path: Path | None, local_test_mode: bool) -> list[str]:
    errors: list[str] = []
    ...
    for producer_id, contract in EVIDENCE_PRODUCERS.items():
        ...
        if local_test_mode and producer_id in {"finalizer"}:
            continue
        ...
        if paths["meta"].is_file():
            try:
                ...
                if metadata.get("candidate_sha") != expected and not local_test_mode:
...
def run(contracts_dir: Path = PHASE5_CONTRACTS, acceptance_path: Path | None = None, evidence_dir: Path | None = None, output_path: Path | None = None, *, expected_candidate_sha: str | None = None, validation_result_path: Path | None = None, local_test_mode: bool = False) -> dict[str, Any]:
    if evidence_dir is None or acceptance_path is None or expected_candidate_sha is None:
        raise RuntimeError("FINALIZER_STOP:required evidence, acceptance, and expected candidate identity are mandatory")
    errors = _errors(contracts_dir, acceptance_path, evidence_dir, expected_candidate_sha, validation_result_path, local_test_mode)
    if errors:
    ...
    summary = {
        "version": 2, "result": "PASS", "classifier_version": CLASSIFIER_VERSION, "rules_version": RULES_VERSION,
        ...
        "synthetic_only": True, "handoff_state": "READY_FOR_INDEPENDENT_PHASE5_ACCEPTANCE",
    }
    ...
    final_paths["meta"].write_text(json.dumps({"producer_id": "finalizer", "candidate_sha": expected_candidate_sha, "exit_code": 0}, sort_keys=True) + "\n", encoding="utf-8")
    final_paths["result"].write_text(json.dumps({"producer_id": "finalizer", "result": "PASS"}, sort_keys=True) + "\n", encoding="utf-8")
