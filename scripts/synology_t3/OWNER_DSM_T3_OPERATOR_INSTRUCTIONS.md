# ProposalOps SYN-T3 R1.6R2 Owner DSM synthetic-share run

This R1.6R2 handoff certifies only the read-only SMB source lane against one isolated synthetic DSM share. R1.6R1 remains historical independently accepted handoff evidence. R1.6R2 preserves its production PRE-bind repair and corrects only the T3 harness interpretation/evidence for smbprotocol 1.15.0 encrypted-session signing semantics: connection signing policy remains mandatory, encryption remains mandatory, and active encryption supplies session message integrity even when smbprotocol clears `session.signing_required`. It does not authorize real AMEC data, business-share browsing, managed writes, parser/classifier execution, deployment, or production integration. R1.6R2 build PASS and GitHub CI PASS do not authorize DSM.
