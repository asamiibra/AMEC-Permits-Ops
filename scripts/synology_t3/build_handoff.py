...
HANDOFF_VERSION = "R1R6R2R2"
IMAGE_NAMESPACE = "proposalops/syn-t3:r1r6r2r2-"
...
def create_bundle(repo: Path, output: Path, run_id: str, harness_sha: str | None = None) -> Path:
    ...
    write_json(bundle / "00_AUTHORIZATION.json", {"workstream": "SYN-T3-COMBINED", "handoff_build_id": run_id, "live_run_id": None, "synthetic_only": True, "real_amec_data_authorized": False, "accepted_v23_sha": ACCEPTED_V23, "owner_dsm_mutations": ["dedicated synthetic share", "two non-admin test identities", "synthetic fixtures", "disable test identities after run"]})
    ...
    write_json(bundle / "06_IMAGE_BUILD_POLICY.json", {"platform": "linux/amd64", "base_image": BASE_IMAGE, "credentials_in_image": False, "business_content_in_image": False, "application_sha": ACCEPTED_V23, "harness_sha": harness_sha, "handoff_build_id": run_id, "live_run_id": None, "handoff_version": HANDOFF_VERSION, "image_ref": f"{IMAGE_NAMESPACE}{harness_sha[:12]}", "image_id": None, "image_tar_sha256": None, "labels": {HARNESS_LABEL: harness_sha, APP_LABEL: ACCEPTED_V23, "org.opencontainers.image.synthetic-only": "true"}})
