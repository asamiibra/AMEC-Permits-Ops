...
check_cat repository R005 bash -c '[[ "$(git branch --show-current)" == azure-sql-foundation-prephase5-r2-v1 ]]'
...
check_cat repository R011 bash -c '[[ -n "$(git show -s --format=%P "$BASE_SHA")" ]]'
...
check_cat workflow W001 has 'azure-sql-foundation-prephase5-r2-v1' "$WORKFLOW"
check_cat workflow W002 count_fixed_is 'azure-sql-foundation-prephase5-r2-v1' "$WORKFLOW" 2
