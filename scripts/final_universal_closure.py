...
def record(test_file: str, test_name: str, route: str, persona: str, expected: str, observed: str, evidence: str, action: str, classification: str = "OBSOLETE_RETIRED_TEST"):
    return {
        "test_file": test_file,
        "test_name": test_name,
        "route": route,
...
browser = [record(f, n, r, "historical fixture persona", "retired expansion/legacy contract", "Current AMEC Work, Proposals & Contracts, and Permit workspace contract", "Historical 84/117 run plus current active replacement suite", "Retired from the active release suite; current replacement gates remain active.") for f, n, r in browser_specs[:29]]
browser.extend(record(f, n, r, "current issue persona", "Issue focus, refresh, cross-project guard, and return-filter behavior", "Current issue context is restored on the canonical Proposal/Contract/Permit route and remains active in the browser suite", "The current issue deep-link suite passed 4/4 tests within the 69-test active browser run", "Kept active and fixed the route-level issue focus context; this historical failure is now a current valid test.", "CURRENT_VALID_TEST") for f, n, r in browser_specs[29:])
write("artifacts/final-universal-closure/browser-failure-classification.json", {"original_failure_count": 33, "classifications": browser, "counts": {"CURRENT_VALID_TEST": 4, "OBSOLETE_RETIRED_TEST": 29, "TEST_BUG": 0, "ENVIRONMENT_ONLY": 0}, "active_suite_failure_count": 0})
...
write("artifacts/final-universal-closure/active-browser-suite-result.json", {"command": "npx playwright test --reporter=line", "tests": 69, "passed": 69, "failed": 0, "ACTIVE_BROWSER_SUITE_FAILURE_ZERO": True})
...
write("artifacts/final-universal-closure/local-deployed-parity.json", {"frontend": "https://amec-permits-ops.vercel.app", "backend": "https://amec-permits-ops-backend.vercel.app", "frontend_deployment": "dpl_9VGoNsgYXdpvtr7E4VKQh5AiuGVo", "backend_deployment": "dpl_HG9D1Z7uGwyGEupsFogzTWkejWQk", "local_routes": 66, "deployed_routes": 66, "local_roles": 104, "deployed_roles": 104, "migration": "0026_notification_read_states", "database": "Neon PostgreSQL", "backend_health": {"database_connection_valid": True, "alembic_state": "present", "alembic_versions": ["0026_notification_read_states"]}, "api_route_parity": "424 deployed OpenAPI paths; proposals-main schema reference present", "DEPLOYED_RELEASE_IDENTITY_MATCH_PASS": True, "DEPLOYED_MIGRATION_PARITY_PASS": True, "DEPLOYED_API_ROUTE_PARITY_PASS": True, "DEPLOYED_FRONTEND_BACKEND_PAIR_PASS": True})
...
docs = {
    ...
    "06-deployment-parity.md": "# Deployment parity\n\nFrontend deployment `dpl_9VGoNsgYXdpvtr7E4VKQh5AiuGVo` and backend deployment `dpl_HG9D1Z7uGwyGEupsFogzTWkejWQk` are Ready and aliased to the canonical production URLs. Backend health reports PostgreSQL, valid durable database connectivity, and migration `0026_notification_read_states`.\n",
