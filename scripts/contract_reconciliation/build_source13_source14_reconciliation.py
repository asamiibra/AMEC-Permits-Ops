...
SUBJECT_SOURCE_SHA = "98bb7a7c88f2407150f0b4796ebf1ed8bdde69fb"
SUBJECT_SOURCE_TREE = "e7af0e31079b1021e8f15136c2089520f81644ee"
...
def make_row(source: str, meta: dict[str, object], table_number: int, row_number: int, cells: list[str], status: str, *, stream: str, precedence_relationship: str = "CURRENT_SOURCE_REQUIREMENT") -> dict[str, object]:
    text = " | ".join(cells)
    ...
    return {
        ...
        "precedence_relationship": precedence_relationship,
...
def source13_rows(meta: dict[str, object]) -> list[dict[str, object]]:
    ...
    for table_number in range(1, 10):
        for row_number, row in enumerate(doc.tables[table_number - 1].rows, 1):
            ...
            rows.append(make_row("Source13", meta, table_number, row_number, cells, row_status(cells[3]), stream="standard_invoice", precedence_relationship="RAW_SOURCE13_PROVISIONAL_ROW_INVENTORY_NOT_FINAL_GOVERNING_LEDGER"))
    if len(rows) != 116:
...
def main() -> None:
    ...
    checkpoint = {
        "checkpoint_scope": "SOURCE13_SOURCE14_INTERMEDIATE_RECONCILIATION",
        "g5_current_contract_closed": False,
        "g6_started": False,
        "latest_governing_contract_source": "SOURCE15",
        "source15_reconciliation_required": True,
        "source14_checkpoint_preserved": True,
        "source15_now_governs_current_g5": True,
        "source15_implementation_not_claimed_in_pr15": True,
        "g5_business_v1_code_frozen_current_contract": False,
        "g6_data_platform_qualified": False,
        "evidence_container_commit": None,
        "evidence_container_commit_recorded_externally": True,
        "pr_reviewed_head_recorded_externally": True,
        "merged_release_sha_recorded_after_merge": True,
    }
    write_json("source13-requirements-ledger.json", {**checkpoint, "ledger": "BUSINESS_V1_RAW_SOURCE13_PROVISIONAL_ROW_INVENTORY_NOT_FINAL_GOVERNING_LEDGER", "source_pinned": True, "source_meta": meta13, "requirement_count": len(rows13), "requirement_orphans": 0, "requirements": rows13})
    write_json("source14-requirements-ledger.json", {**checkpoint, "ledger": "BUSINESS_V1_CURRENT_CONTRACT_SOURCE14", "source_pinned": True, "source_meta": meta14, "primary_requirement_count": len(rows14_primary), "finance_revalidation_enumerated_count": len(rows14_finance), "explicit_requirement_count": len(rows14_primary) + len(rows14_finance), "requirement_orphans": 0, "primary_requirements": rows14_primary, "finance_revalidation_requirements": rows14_finance})
