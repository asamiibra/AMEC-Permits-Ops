...
def main() -> None:
    ...
    contract_integrity = {
        ...
        "duplicate_part_numbers": len(files) - len({re.search(r"Part_(\d+)_of_10", path.name).group(1) for path in files}),
        "missing_part_numbers": sorted(set(str(i) for i in range(1, 11)) - {re.search(r"Part_(\d+)_of_10", path.name).group(1) for path in files}),
        "major_section_order": "PASS" if [re.search(r"Part_(\d+)_of_10", path.name).group(1) for path in files] == [str(i) for i in range(1, 11)] else "FAIL",
