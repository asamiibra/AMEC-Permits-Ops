...
def main() -> None:
    ...
    azure_runtime = json.loads((OUT / "15-azure-runtime-acceptance.json").read_text(encoding="utf-8"))
    azure_runtime["health"] = {"status": 200, "environment": "AZURE-PREPROD", "synthetic_only": True, "real_data_allowed": None, "database_dialect": "mssql", "database_connection_valid": False, "alembic_versions": [], "runtime_release_sha": None, "migration_head": None}
    azure_runtime["health_live"] = "NOT_REQUERIED_IN_THIS_BRANCH_RUN"
    azure_runtime["health_ready"] = "NOT_REQUERIED_IN_THIS_BRANCH_RUN"
    azure_runtime["current_read_only_observation"] = "Fresh GET /health returned HTTP 200 with database_connection_valid=false and no migration versions/runtime release SHA."
    azure_runtime["artifact_sha256"] = sha256(json.dumps({key: value for key, value in azure_runtime.items() if key != "artifact_sha256"}, sort_keys=True, separators=(",", ":")).encode())
    write("15-azure-runtime-acceptance.json", azure_runtime)
