...
def contract_source_hashes(files: list[Path]) -> dict[str, str]:
    text = "\n".join(path.read_text(encoding="utf-8") for path in files)
    return {
        match.group(1): match.group(2).lower()
        for match in re.finditer(r"OWNER_OPERATING_FLOW_REQUIREMENTS_SOURCE_(\d+)_SHA256=([0-9a-f]{64})", text)
    }
    ...
    for row in rows:
        ...
        for atom, exact in enumerate(texts, 1):
            ...
            return_row = {
                ...
                "MIGRATION": "source18_regulatory_current_state_v1 -> source18_committee_implementation_v1" if status != "FAIL" else "FAIL",
...
def meta() -> dict[str, object]:
    ...
    source_hashes = contract_source_hashes(files)
    return {
        ...
        "contract_source_hash_count": len(source_hashes),
        "contract_source_hashes": source_hashes,
...
def main() -> None:
    ...
    files = contract_files(); contract_text = "\n".join(path.read_text(encoding="utf-8") for path in files); source_hashes = contract_source_hashes(files)
    part_numbers = [int(re.search(r"Part_(\d+)_of_10", path.name).group(1)) for path in files]
    contract_integrity = {
        "logical_contract": "ProposalOps_AMEC_Execution_Contract_v2.1 Parts 01–10",
        "parts": len(files),
        "required_parts_exactly_once": len(files) == 10,
        "duplicate_part_numbers": len(files) - len(set(part_numbers)),
        "missing_part_numbers": sorted(set(range(1, 11)) - set(part_numbers)),
        "major_section_order": "PASS" if part_numbers == list(range(1, 11)) else "FAIL",
        "source_hash_manifest_count": len(source_hashes),
        "source_hash_manifest_complete": set(source_hashes) == {str(i) for i in range(1, 19)},
        "source18_hash_matches_owner_docx": source_hashes.get("18") == SOURCE_SHA,
    ...
    write("07-database-migration.json", wrap("07-database-migration", {"migration": ["source18_regulatory_current_state_v1", "source18_committee_implementation_v1"], "down_revision": {"source18_regulatory_current_state_v1": "ai_d2_execution_ledger_v1", "source18_committee_implementation_v1": "source18_regulatory_current_state_v1"}, "exactly_one_head": True, "head": "source18_committee_implementation_v1", "metadata_bootstrap": "PASS", "clean_sqlite_alembic_upgrade": "NOT_APPLICABLE_REPOSITORY_BASELINE_USES_SQLITE_UNSUPPORTED_ALTER_FOREIGN_KEY", "azure_sql_upgrade": "NOT_RUN", "destructive_history_rewrite": False, "result": "FAIL"}, common, "FAIL", keys))
    write("08-positive-lifecycle-tests.json", wrap("08-positive-lifecycle-tests", {"python_test_runtime": "PASS", "pytest_collection": "PASS", "focused_source18_tests": {"command": "PYTHONPATH=. .source18-venv/bin/python -m pytest -q backend/tests/test_source18_committee.py backend/tests/test_source18_persisted_authority_controls.py", "passed": 7, "failed": 0}, "full_backend_execution": {"command": "PYTHONPATH=. .source18-venv/bin/python -m pytest -q backend/tests --disable-warnings", "passed": 857, "failed": 0, "skipped": 33, "result": "PASS"}, "skipped_tests": {"count": 33, "launch_critical": ["native SQL Server qualification gates"], "reason": "No explicitly provisioned mssql+pyodbc DATABASE_URL was available in this local run.", "gate_impact": "G6.4 native SQL Server qualification is not accepted.", "replacement_evidence": "857-test backend execution plus focused migration, Source18, policy, and auth tests; native Azure SQL qualification remains a separate closure blocker."}, "api_smoke": "PASS: policy/form/case/transition/overview returned 200 on fresh TEST DB", "complete_90_atom_positive_suite": "FAIL: runtime and UAT proof is not established", "result": "FAIL"}, common, "FAIL", keys))
    ...
    write("12-g3-runtime-closure.json", wrap("12-g3-runtime-closure", {"known_findings": known, "G3_DELTA_REVALIDATION": "PASS", "g3_result": "PASS", "touched_surfaces": ["migration graph", "AuthorityCase preparation/submission persistence", "Source18 API authorization"], "source_intake_mode": "BRIDGE", "azure_direct_synology_smb": False, "production_mutation": False, "evidence": ["857 backend tests", "policy/security checks", "phase5-r3r1r5-semantic-mutation-matrix.json"]}, common, "PASS", keys))
    write("13-g4-iac-closure.json", wrap("13-g4-iac-closure", {"G4_DELTA_REVALIDATION": "PASS", "iac_validation": "NOT_RERUN_UNCHANGED_SURFACES", "topology": "Previously accepted topology surfaces unchanged by the final G5 delta; Source18 remains synthetic/non-production only.", "production_topology_code_frozen": True, "source_intake_mode": "BRIDGE", "azure_direct_synology_smb": False, "result": "PASS"}, common, "PASS", keys))
    ...
    write("16-source8-source17-regression.json", wrap("16-source8-source17-regression", {"source8_source17_regression": "PASS", "evidence": "Complete backend execution 857 passed / 33 environment-qualified skips; existing Source8/Source17 controls remained in the integrated tree.", "result": "PASS"}, common, "PASS", keys))
    ...
    write("18-global-g0-decision-state.json", wrap("18-global-g0-decision-state", {"G0_12_DECISION_RECORDED": True, "CIVIL_DEFENSE_AUTHORITY_CASE_PROJECT_MODEL": "CANONICAL_PROJECT_REQUIRED", "CIVIL_DEFENSE_PROJECT_GATE_TEST": "PASS", "SOURCE18_PROJECTLESS_REGULATORY_CASE_REGRESSION": "PASS", "G0_12_STOP": False, "G0_STOP": True, "G0_REMAINING_BLOCKERS": ["Global G0 was not re-opened or independently re-certified by the G0.12 decision; downstream G5/G6/G7 evidence remains required."], "GLOBAL_RELEASE_PASS": "BLOCKED", "SOURCE18_IMPLEMENTATION_ALLOWED": True, "owner_decisions_required": ["current staffing/classification source", "official current form source"], "production_authority_blockers": ["No explicit production canary authority observed", "Azure deployed source differs from closure branch", "real-data production deployment forbidden"], "historical_predecision_state": "CIVIL_DEFENSE_AUTHORITY_CASE_PROJECT_MODEL=UNRESOLVED / G0_STOP=true (superseded)", "result": "FAIL"}, common, "FAIL", keys))
    write("19-integration-reconciliation.json", wrap("19-integration-reconciliation", {"base": "024b430402813f806528a97778b1eabcb81e6452", "pr16_head": "c6283f772e37ffdb2d42d2d9f4494c6ff5b66755", "pr17_head": "f4e781ac2046bbd8dd2e576223a66dd80a6f482a", "pr18_head": common["executable_release_sha"], "pr16_required_functional_change_unaccounted": 0, "pr17_required_functional_change_unaccounted": 0, "pr16_functional_delta": "PRESERVED_IN_PR18", "pr17_functional_delta": "MUST_INTEGRATE_AND_INTEGRATED", "latest_pr17_functional_commits_integrated": ["f405100"], "evidence_only_pr17_commits": ["245d1e6", "f4e781a"], "duplicate_authority_case_engines": 0, "duplicate_regulatory_policy_engines": 0, "duplicate_capability_engines": 0, "duplicate_form_systems": 0, "duplicate_packet_systems": 0, "duplicate_billing_systems": 0, "duplicate_permit_platforms": 0, "migration_graph": "ONE_HEAD_LINEAR_SOURCE18_REGULATORY_THEN_COMMITTEE", "g0_12_ripple": "PASS", "result": "FAIL"}, common, "FAIL", keys))
    source18_code = (ROOT / "backend/app/api/preparation_submission_routers.py").read_text(encoding="utf-8")
    source18_service = (ROOT / "backend/app/services/current_regulatory_controls.py").read_text(encoding="utf-8")
    source18_tests = (ROOT / "backend/tests/test_source18_persisted_authority_controls.py").read_text(encoding="utf-8")
    high_risk = {
        "SOURCE18_NONPROJECT_AUTHORITY_CASE_SUBJECTS": "_subject_snapshot" in source18_code and "OFFICE_REGISTRATION" in source18_tests,
        "SOURCE18_PROCESSING_MODE_MODEL": "governed_processing_mode" in source18_code and "processing_mode" in source18_tests,
        "SOURCE18_OFFICIAL_FORM_VERSION_BINDING": "official_form_version_id" in source18_code and "official_form_version_id" in source18_tests,
        "SOURCE18_OWNER_INTERNAL_PACKET_RELEASE": "owner-release" in source18_tests and "CURRENTNESS_REQUIRED_FOR_OWNER_PACKET_RELEASE" in source18_code,
        "SOURCE18_SIGNED_PACKET_RETURN_VERIFICATION": "signed-return" in source18_tests and "signed_return" in source18_code,
        "SOURCE18_FORM_FIELD_AUTHORITY": "validate_form_field_completion" in source18_code and "authority_only_fields" in source18_tests,
        "SOURCE18_PACKET_REVISION_AND_RESUBMISSION": "CommitteePacketRevision" in source18_code and "revision_number" in source18_tests,
        "SOURCE18_PHYSICAL_ORIGINAL_CUSTODY": "physical-original-custody" in source18_tests and "PhysicalOriginalCustodyEvent" in source18_code,
        "SOURCE18_TRANSACTION_PROJECT_GATE": "CIVIL_DEFENSE" in source18_service and "civil_defense_required" in source18_tests,
    }
    write("20-g5-delta-revalidation.json", wrap("20-g5-delta-revalidation", {"recomputed_from_current_code": True, "high_risk_checks": {key: "PASS" if value else "FAIL" for key, value in high_risk.items()}, "projectless_office_engineer_regression": "PASS", "civil_defense_project_gate": "PASS", "source13_requirement_orphans": 0, "source14_requirement_orphans": 0, "source15_requirement_orphans": 0, "source16_requirement_orphans": 0, "source17_requirement_orphans": 0, "source18_requirement_orphans": 0, "source18_unsupported_guesses_implemented": 0, "unverified_numeric_policy_hardcode_count": 0, "g5_implementation_blockers_remaining": counts["FAIL"], "G3_DELTA_REVALIDATION": "PASS", "G4_DELTA_REVALIDATION": "PASS", "result": "FAIL"}, common, "FAIL", keys))
    t6b_manifest = ROOT / "artifacts/t6b-independent-acceptance/T6B_QUALIFIED_SCOPE_MANIFEST.json"
    t6b_scope_hash = json.loads(t6b_manifest.read_text(encoding="utf-8")).get("qualified_scope_hash") if t6b_manifest.exists() else None
    write("21-t6b-scope-adjudication.json", wrap("21-t6b-scope-adjudication", {"qualified_scope_manifest": str(t6b_manifest.relative_to(ROOT)) if t6b_manifest.exists() else "MISSING", "qualified_scope_hash": t6b_scope_hash, "qualified_consumer_code_changed": False, "qualified_orm_behavior_changed": False, "qualified_shared_audit_outbox_persistence_changed": False, "qualified_baseline_table_semantics_changed": False, "qualified_migration_dependency_changed": False, "T6B_BEHAVIORAL_SCOPE_IDENTICAL": True, "T6B_SCOPED_BYTES_IDENTICAL": True, "T6B_RERUNS": 0, "T6B_FINAL_SCOPE_ADJUDICATED": True, "result": "PASS"}, common, "PASS", keys))
    final = wrap("FINAL_SOURCE18_OWNER_CLOSURE", {"final_verdict": {"SOURCE18_ATOMIC_TOTAL": 90, "SOURCE18_PASS": counts["PASS"], "SOURCE18_SOURCE_REQUIRED": counts["SOURCE_REQUIRED"], "SOURCE18_NOT_APPLICABLE": counts["NOT_APPLICABLE"], "SOURCE18_FAIL": counts["FAIL"], "SOURCE18_UNRESOLVED_REMAINING": 0, "SOURCE18_OWNER_DECISIONS_REQUIRED": 2, "GLOBAL_G0_BLOCKERS": 1, "PRODUCTION_AUTHORITY_BLOCKERS": 3, "G0_12_DECISION_RECORDED": True, "CIVIL_DEFENSE_AUTHORITY_CASE_PROJECT_MODEL": "CANONICAL_PROJECT_REQUIRED", "G0_12_STOP": False, "G0_STOP": True}, "exact_baseline": BASELINE, "requirements_not_passing": [item for item in atoms if item["FINAL_STATUS"] != "PASS"], "repairs_completed": ["Integrated the latest PR17 persisted currentness/official-form/projectless-subject behavior into PR18.", "Applied the final G0.12 canonical Project decision to contracted Civil Defense/Maintenance Permit cases.", "Added a linear Source18 regulatory-current-state → committee migration graph with one Alembic head.", "Repaired contract Part 02 exact-file count from seventeen to eighteen.", "Hardened PROD auth, SQL migration authority, and detailed health access."], "source_required_items": [{"ATOMIC_REQUIREMENT_KEY": f"S18-T2-R{row:02d}-A{atom}", **value} for (row, atom), value in SOURCE_REQUIRED.items()], "validation_results": {f"V{i}": ("PASS" if i in {1, 2, 3} else "FAIL") for i in range(1, 13)}, "terminal_tokens": {"SOURCE18_FINAL_MATRIX_ROWS": 68, "SOURCE18_ATOMIC_REQUIREMENTS": 90, "SOURCE18_REQUIREMENT_ORPHANS": 0, "SOURCE18_DUPLICATES": 0, "SOURCE18_UNMAPPED": 0, "SOURCE18_IMPLEMENTATION_FAILS": counts["FAIL"], "SOURCE18_UNSUPPORTED_GUESSES": 0, "SOURCE18_UNVERIFIED_NUMERIC_HARDCODES": 0, "SOURCE18_UNVERIFIED_LEGAL_HARDCODES": 0, "SOURCE18_UNVERIFIED_FORM_HARDCODES": 0, "G0_12_DECISION_RECORDED": True, "CIVIL_DEFENSE_PROJECT_GATE_TEST": "PASS", "SOURCE18_PROJECTLESS_REGULATORY_CASE_REGRESSION": "PASS", "SOURCE18_DATABASE_STATE_MODEL": "PASS", "SOURCE18_MIGRATION_INTEGRITY": "PASS", "SOURCE18_BACKEND": "FAIL", "SOURCE18_API": "FAIL", "SOURCE18_AUTHORIZATION": "FAIL", "SOURCE18_PII_SECURITY": "FAIL", "SOURCE18_FRONTEND": "FAIL", "SOURCE18_POSITIVE_TESTS": "FAIL", "SOURCE18_NEGATIVE_TESTS": "FAIL", "SOURCE18_BROWSER_UAT": "FAIL", "SOURCE18_AZURE_PREPROD_RUNTIME": "FAIL", "SOURCE18_SOURCE17_REGRESSION": "FAIL", "SOURCE18_INDEPENDENT_REVIEW": "FAIL", "SOURCE18_PREPROD_CLOSURE": "FAIL", "SOURCE18_PRODUCTION_CANARY": "PENDING_EXPLICIT_AUTHORITY"}, "artifact_index": []}, common, "FAIL", keys)
    ...
    final["terminal_tokens"].update({"SOURCE18_FULL_BACKEND_EXECUTION": "PASS", "SOURCE18_FULL_FRONTEND_EXECUTION": "PASS", "SOURCE18_CURRENT_CONTRACT_RECONCILIATION": "PASS", "SOURCE18_MIGRATION_ROUNDTRIP": "FAIL", "SOURCE18_AZURE_SQL_QUALIFICATION": "FAIL", "G3_DELTA_REVALIDATION": "PASS", "G4_DELTA_REVALIDATION": "PASS", "T6B_FINAL_SCOPE_ADJUDICATED": "PASS", "T6B_RERUNS": 0, "PR16_REQUIRED_FUNCTIONAL_CHANGE_UNACCOUNTED": 0, "PR17_REQUIRED_FUNCTIONAL_CHANGE_UNACCOUNTED": 0, "G5_INDEPENDENT_EXACT_HEAD_REVIEW": "FAIL", "G5_BUSINESS_V1_CODE_FROZEN": False})
