...
test("Proposals & Contracts navigation remains reachable", async ({ page }) => { await page.goto("/work"); await page.getByRole("navigation").getByRole("button", { name: "Proposals & Contracts" }).click(); await expect(page).toHaveURL(/\/proposals-contracts$/); });
