...
test("Contract & Mobilization remains reachable from Home", async ({ page }) => { await page.goto("/home"); await page.getByRole("navigation").getByRole("button", { name: "Contract & Mobilization" }).click(); await expect(page).toHaveURL(/\/proposals-contracts$/); });
