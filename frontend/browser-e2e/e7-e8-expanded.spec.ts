...
test("Contract & Mobilization remains reachable from Home", async ({ page }) => { await page.goto("/home"); await page.getByRole("navigation").getByRole("button", { name: "Contract & Mobilization" }).click(); await expect(page).toHaveURL(/\/contract-mobilization$/); });
test("Issues remains reachable as a preserved direct route", async ({ page }) => { await page.goto("/issues"); await expect(page).toHaveURL(/\/issues$/); await expect(page.getByText(/Issues across AMEC work|Commercial & project issues|Engineering issues/).first()).toBeVisible(); });
test("Issues and Notifications remain separate preserved surfaces", async ({ page }) => { await page.goto("/issues"); await expect(page).toHaveURL(/\/issues$/); await page.goto("/notifications"); await expect(page).toHaveURL(/\/notifications$/); await expect(page.getByText(/notifications/i).first()).toBeVisible(); });
