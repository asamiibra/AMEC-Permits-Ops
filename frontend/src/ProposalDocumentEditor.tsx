...
type GenerationSummary = { state?: string; published_ai_mutation_count?: number; owner_mutation_count?: number; generation_review_reason?: string | null; generated_from_manifest_hash?: string; validation_state?: string; published_mutations?: any[]; no_ai_changes_required?: boolean };
type RevisionData = { revision_number?: number; content_hash?: string; change_plan?: Record<string, any>; owner_change_plan?: Record<string, any>; ai_provenance?: Record<string, any>; generation_summary?: GenerationSummary; source_set_hash?: string };
...
export function ProposalDocumentEditor({ role = "OWNER_SPONSOR", proposalId, revisionId, onBack }: { role?: string; proposalId?: string; revisionId?: string; onBack?: () => void }) {
  ...
  const [publishState, setPublishState] = useState<"IDLE" | "CONFIRM" | "PUBLISHING" | "PUBLISHED" | "ERROR">("IDLE");
  const load = async () => {
    if (!file) return;
  ...
  useEffect(() => {
    ...
    const loadCanonical = async () => {
      ...
      try {
        ...
        try {
          const proposal = await api<any>(`/api/bd/proposals/${proposalId}`, { headers: headers(role) });
          if (!cancelled && proposal?.proposal_reference) setProposalReference(String(proposal.proposal_reference));
        } catch { /* the editor remains usable when the optional register projection is unavailable */ }
  ...
  const generationSummary = revisionData?.generation_summary || {};
  const mutations = Array.isArray(generationSummary.published_mutations) ? generationSummary.published_mutations : (Array.isArray(plan.mutations) ? plan.mutations : []);
  const generationCoverage = Array.isArray(plan.generation_coverage) ? plan.generation_coverage : [];
  ...
  const acceptAndPublish = async () => {
    if (!proposalId || publishState !== "CONFIRM") return;
    setPublishState("PUBLISHING"); setStatus("Publishing the exact reviewed Proposal revision…");
    try {
      await api(`/api/bd/proposals/${proposalId}/accept`, { method: "POST", headers: headers(role) });
      setPublishState("PUBLISHED");
      setStatus("Proposal accepted and published by the Owner.");
      onBack?.();
    } catch (error) {
      setPublishState("ERROR");
      setStatus(error instanceof Error ? error.message : "Proposal could not be accepted and published.");
    }
  };
  ...
  const renderNode = (node: EditorNode, index: number) => <div key={node.id} className={`proposal-editor-node ${node.editable ? "editable" : "readonly"}`} data-anchor={node.anchor}><small>{node.editable ? "Proposal text" : node.block_type === "COMPLEX_PARAGRAPH" ? "Table / complex content · preserved in Document view" : node.block_type === "DRAWING" || node.block_type === "EMBEDDED_MEDIA" || node.block_type === "VML_TEXTBOX" ? "Image / media · preserved in Document view" : "Preserved document content"}</small>{node.editable ? <div dir="auto" className="bidi-content" contentEditable suppressContentEditableWarning role="textbox" aria-label={`Editable paragraph ${index + 1}`} onInput={(event) => { const text = event.currentTarget.textContent || ""; setSaveState("IDLE"); setShowOwnerReview(false); setNodes((items) => items.map((item) => item.id === node.id ? { ...item, text } : item)); }}>{node.text}</div> : <BidiText as="div" aria-label="Preserved document block">{node.text || "Preserved content"}</BidiText>}</div>;
  ...
  return <section className="panel proposal-editor" aria-label="Proposal document editor">
    ...
    <div className="proposal-editor-toolbar">{canonical ? <span className="muted">Canonical server revision · generated DOCX loaded and verified</span> : <><label>Open baseline DOCX<input type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label><button className="button-secondary" disabled={!file || busy} onClick={() => void load()}>Import</button></>}{model && canonical && mode === "DOCUMENT" && <><button className="button-secondary" disabled={busy} onClick={() => void retryRender()}>{renderState === "RENDERING" ? "Rendering…" : "Refresh render"}</button><button className="button-primary" disabled={busy} onClick={() => void downloadDocx()}>{downloadState === "RUNNING" ? "Preparing DOCX…" : downloadState === "DONE" ? "Downloaded ✓" : "Download DOCX"}</button>{(role === "OWNER_SPONSOR" || role === "SYSTEM_ADMIN") && <button className="button-secondary" disabled={busy || sectionDirty || publishState === "PUBLISHING" || publishState === "PUBLISHED"} onClick={() => setPublishState("CONFIRM")}>{publishState === "PUBLISHED" ? "Published ✓" : "Accept & Publish Proposal"}</button>}</>}{model && canonical && mode === "EDIT_SECTIONS" && <><button className="button-secondary" disabled={busy} onClick={() => void reviewOwnerEdits()}>Review my edits</button><button className="button-primary" disabled={busy || !sectionDirty} onClick={() => void saveSection()}>{saveState === "SAVING" ? "Saving…" : saveState === "SAVED" ? "Saved ✓" : "Save section"}</button></>}</div>
    {publishState === "CONFIRM" && <section className="proposal-publish-confirm" role="dialog" aria-label="Confirm Proposal publication"><b>Accept &amp; Publish Proposal</b><p>Pin the exact current document revision as the final Proposal V1 artifact?</p><dl><div><dt>Proposal reference</dt><dd>{proposalReference || proposalId || "Unavailable"}</dd></div><div><dt>Revision</dt><dd>R{revisionData?.revision_number ?? "—"}</dd></div><div><dt>Source manifest</dt><dd>{sourceState === "STALE_SOURCE_MANIFEST" ? "Stale" : "Current"}</dd></div><div><dt>Document hash</dt><dd>{revisionData?.content_hash || "Recorded on the server"}</dd></div></dl><div className="proposal-publish-actions"><button className="button-secondary" onClick={() => setPublishState("IDLE")}>Cancel</button><button className="button-primary" onClick={() => void acceptAndPublish()}>Confirm &amp; Publish</button></div></section>}
    {status && <div className="inline-message" role="status"><BidiText>{status}</BidiText></div>}
    {canonical && <><div className="proposal-editor-meta"><span>Revision <b>R{revisionData?.revision_number ?? "—"}</b></span><span>Generation <b>{String(generationSummary.state || provenance.generation_state || sourceState || "READY_FOR_EDIT").replaceAll("_", " ")}</b></span><span>Sources <b>{Array.isArray(provenance.evidence_refs) ? provenance.evidence_refs.length : sources.filter((source) => source.included_in_proposal !== false).length}</b></span><span>AI changes <b>{generationSummary.published_ai_mutation_count ?? mutations.length}</b></span><span>Owner edits <b>{generationSummary.owner_mutation_count ?? 0}</b></span><span>Manifest <b>{sourceState === "STALE_SOURCE_MANIFEST" || (sourceManifestHash && revisionData?.source_set_hash && sourceManifestHash !== revisionData.source_set_hash) ? "stale" : "current"}</b></span>{sourceState === "STALE_SOURCE_MANIFEST" && <span className="source-stale">Source changes available</span>}</div><nav className="proposal-editor-modes" aria-label="Proposal V1 document modes">{([["DOCUMENT", "Document"], ["EDIT_SECTIONS", "Edit Sections"], ["AI_CHANGES", "AI Changes"], ["SOURCES", "Sources"]] as Array<[EditorMode, string]>).map(([value, label]) => <button key={value} className={mode === value ? "active" : ""} onClick={() => requestNavigation({ mode: value })}>{label}</button>)}</nav></>}
    ...
    {mode === "EDIT_SECTIONS" && model && <div className="proposal-sections-layout"><nav className="proposal-section-nav" aria-label="Proposal sections">{sections.map((section) => <button key={section.section_id} className={selectedSection?.section_id === selectedSectionId ? "active" : ""} onClick={() => requestSection(section.section_id)}><BidiText>{section.title}</BidiText><small>{section.level ? `Level ${section.level}` : "General content"}</small></button>)}</nav><section className="proposal-section-editor"><div className="proposal-section-heading"><div><span className="eyebrow">{selectedSection?.level ? `SECTION ${selectedSection.level}` : "PROPOSAL CONTENT"}</span><BidiText as="h4">{selectedSection?.title || "Document body"}</BidiText></div><span className="muted">{sectionDirty ? "Unsaved changes" : "Saved ✓"}</span></div><div className="proposal-editor-canvas">{selectedNodes.map(renderNode)}</div>{showOwnerReview && <aside className="proposal-owner-review" aria-label="My edits"><h4>Review my edits</h4>{changes.length ? changes.map((change: any) => <article className="proposal-editor-change" key={change.anchor}><BidiText as="del">{change.before || "(empty)"}</BidiText><BidiText as="ins">{change.after || "(empty)"}</BidiText>{change.review_state === "KEPT" ? <span className="document-operation-state">Kept</span> : <div><button className="text-button" onClick={() => keepOwnerChange(String(change.anchor))}>Keep change</button><button className="text-button" onClick={() => revertOwnerChange(change)}>Revert</button></div>}</article>) : <p className="muted">No unsaved changes in this section.</p>}</aside>}<div className="proposal-section-actions"><button className="button-secondary" disabled={!selectedSection || sections.findIndex((section) => section.section_id === selectedSection.section_id) <= 0} onClick={() => { const index = sections.findIndex((section) => section.section_id === selectedSection.section_id); if (index > 0) requestSection(sections[index - 1].section_id); }}>Previous section</button><button className="button-secondary" disabled={!selectedSection || sections.findIndex((section) => section.section_id === selectedSection.section_id) >= sections.length - 1} onClick={() => { const index = sections.findIndex((section) => section.section_id === selectedSection.section_id); if (index < sections.length - 1) requestSection(sections[index + 1].section_id); }}>Next section</button></div></section></div>}
    {mode === "AI_CHANGES" && <section className="proposal-ai-changes" aria-label="AI changes"><h4>AI Changes</h4><p className="muted">Every baseline section is accounted for below. Published changes include their stored source evidence; preserved sections remain unchanged by design.</p>{generationCoverage.length > 0 && <section className="proposal-ai-section proposal-generation-coverage" aria-label="Generation coverage"><h5>Full document coverage <small>{generationCoverage.length} sections considered</small></h5>{generationCoverage.map((item: any) => <article className="proposal-editor-change" key={String(item.section_id)}><span className="muted">{String(item.disposition || "KEEP_UNCHANGED").replaceAll("_", " ")}</span><b><BidiText>{item.section_title || "Proposal section"}</BidiText></b><BidiText as="p">{item.reason || "Section disposition recorded in the generation plan."}</BidiText>{item.mutation_ids?.length ? <small>{item.mutation_ids.length} published change{item.mutation_ids.length === 1 ? "" : "s"}</small> : null}</article>)}</section>}{mutations.length ? groupedMutations.map(({ section, mutations: sectionMutations }) => <section key={section.section_id} className="proposal-ai-section"><h5><BidiText>{section.title}</BidiText> <small>{sectionMutations.length} AI change{sectionMutations.length === 1 ? "" : "s"}</small></h5>{sectionMutations.map((change: any) => <article className="proposal-editor-change" key={change.anchor}><span className="muted">Published change</span><BidiText as="del">{change.before || change.expected_text || "(no prior text recorded)"}</BidiText><BidiText as="ins">{change.after || change.replacement || "(no replacement recorded)"}</BidiText><BidiText as="p">{change.reason || "Source-grounded Proposal change"}</BidiText><button className="text-button" onClick={() => void showMutationSource(String(change.anchor))}>Show source</button></article>)}</section>) : <div className="inline-message"><b>{generationSummary.state === "NO_AI_CHANGES_REQUIRED" ? "No AI changes required" : "Generation requires review"}</b><p>{generationSummary.state === "NO_AI_CHANGES_REQUIRED" ? "The validated Proposal already matches the selected source evidence." : generationSummary.generation_review_reason || "AI produced no valid document changes."}</p>{generationSummary.state === "GENERATION_REVIEW_REQUIRED" && <p>Review the baseline and source set, then retry generation.</p>}</div>}{mutationEvidence && <div className="proposal-mutation-evidence"><h4>Source evidence</h4>{mutationEvidence.citations?.length ? mutationEvidence.citations.map((citation, index) => <div key={`${citation.citation_key || "citation"}-${index}`}><BidiCode as="b">{citation.filename || "Source file"}</BidiCode><small><BidiText>{categoryLabel(citation.logical_category)} · {citation.document_version_id || "version unavailable"}</BidiText></small><BidiText as="p">{citation.locator?.excerpt || citation.locator?.text || citation.excerpt || citation.locator?.page || "Exact locator retained in the revision evidence."}</BidiText>{(() => { const source = sources.find((item) => item.filename === citation.filename); return source ? <span><button className="text-button" onClick={() => void openSource(source)}>View</button><button className="text-button" onClick={() => void openSource(source, true)}>Download</button></span> : null; })()}</div>) : <p className="muted">No exact citation evidence was recorded.</p>}</div>}</section>}
