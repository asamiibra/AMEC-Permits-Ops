...
export function ProposalDocumentEditor({ role = "OWNER_SPONSOR", proposalId, revisionId, onBack }: { role?: string; proposalId?: string; revisionId?: string; onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    const loadCanonical = async () => {
      ...
      try {
        ...
        const imported = await api<EditorModel>("/api/proposals-v1/editor/import", { method: "POST", headers: headers(role), body: form });
        if (cancelled) return;
        setFile(canonicalFile); setModel(imported); setNodes(imported.nodes); setChanges([]); setRevisionData(revision); setEvidenceRefs(revision.ai_provenance?.evidence_refs || []); setMode("DOCUMENT"); setSelectedSectionId(imported.sections?.[0]?.section_id || "document-body"); setRenderError("");
        let renderNotice = "";
        try {
          ...
          if (!cancelled) { setPreviewUrl(URL.createObjectURL(rendered)); setRenderError(""); }
        } catch (renderError) {
          renderNotice = renderError instanceof Error ? renderError.message : "The generated DOCX could not be rendered.";
          if (!cancelled) { setPreviewUrl(""); setRenderError(renderNotice); }
        }
  ...
  const groupedMutations = sections.map((section) => ({ section, mutations: mutations.filter((mutation: any) => section.node_ids.includes(String(mutation.anchor))) })).filter((group) => group.mutations.length);
  ...
  const saveCanonical = async (): Promise<any | null> => {
    ...
    try {
      ...
      setChanges([]); setReloadNonce((value) => value + 1);
      return result;
  ...
  const retryRender = async () => {
    ...
    try {
      ...
      setPreviewUrl(URL.createObjectURL(rendered)); setRenderError(""); setStatus("Document preview loaded.");
    } catch (error) {
      const message = error instanceof Error ? error.message : "The generated DOCX could not be rendered.";
      setRenderError(message); setStatus(`Document preview unavailable: ${message}`);
    } finally { setBusy(false); }
  };
