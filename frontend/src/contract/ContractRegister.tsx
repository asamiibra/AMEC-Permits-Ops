...
export function ContractRegister({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const load = async (nextLane = lane, nextQuery = query) => {
    ...
    try {
      ...
      setItems(nextItems);
      const proposalData = await api<any>("/api/bd/proposals", { signal: controller.signal });
  ...
  const visibleMessage = useMemo(() => message || (loadState === "EMPTY" ? "No Contracts in this lane." : ""), [loadState, message]);
