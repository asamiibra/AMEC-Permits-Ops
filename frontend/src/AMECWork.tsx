...
export function AMECWorkPage({ openAbout }: { openAbout?: () => void }) {
  ...
  return <div className="workflow-page amec-work-page">
    ...
    <section className="dashboard-destination-grid" aria-label="Dashboards">
      <a className="dashboard-destination-card" href="/content-library"><span className="eyebrow">CONTENT LIBRARY</span><h3>Content Library</h3><p>Governed Forms, Reports, Engineering Works, and Definitions workspace.</p><span className="button-secondary">Open Content Library <Icon name="arrow-up-right" size={14} /></span></a>
    </section>
    ...
    {!loading && !error && data?.recent_changes?.length > 0 && <section className="panel amec-work-changes"><div className="panel-head"><div><span className="eyebrow">RECENT IMPORTANT CHANGES</span><h3>What changed</h3></div><a className="text-button" href="/notifications">View all notifications <Icon name="arrow-up-right" size={14} /></a></div>{data.recent_changes.slice(0, 3).map((change: any) => <a className="amec-work-change" href={canonicalDomainRoute(change.deep_link, "/notifications")} key={change.id}><div><b>{change.title}</b><small>{change.detail}</small></div><span>{change.when ? new Date(change.when).toLocaleDateString() : "Recent"}</span></a>)}</section>}
  </div>;
}

function WorkCard({ item }: { item: any }) {
  const urgency = dueCopy(item);
  return <article className="amec-work-card"><div className="amec-work-card-main"><div className="amec-work-card-top"><span className="amec-domain">{label(item.domain)}</span>{item.blocking && <span className="amec-blocking">Blocking</span>}{urgency && <span className={item.overdue ? "amec-overdue" : "amec-due"}>{urgency}</span>}</div><h4>{item.title}</h4><p>{item.business_context}</p><div className="amec-work-card-meta"><span>{item.reference || "System"}</span><span>{item.assigned_team}{item.assigned_user ? ` · ${item.assigned_user}` : ""}</span>{item.stage && <span>{item.stage}</span>}</div></div><a className="button-secondary" href={canonicalDomainRoute(item.deep_link, "/work")}>{item.cta_label || "Review source"}</a></article>;
}
