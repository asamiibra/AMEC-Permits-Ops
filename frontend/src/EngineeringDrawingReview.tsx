...
export function EngineeringDrawingReviewPage() {
  ...
  useEffect(() => { api<unknown>("/api/projects").then(async response => { const list = Array.isArray(response) ? response as Project[] : []; setProjects(list); const first = list[0]?.id || ""; setProjectId(first); await load(first); }).catch(() => setError("Canonical Project register is unavailable.")); }, []);
  ...
  return <div className="workflow-page expansion-workspace">
    <div className="page-intro"><div><span className="eyebrow">ENGINEERING · DRAWING REVIEW</span><h2>Drawing Review</h2><p>Review Category, exact business revision, pinned Drawing/PDF evidence, internal comments, and professional findings.</p></div><span className="tag">Human-controlled</span></div>
    ...
    <section className="panel expansion-context"><div><b>Project context</b><small>Every review, rendition, comment, assignment, and finding is project-scoped.</small></div><select aria-label="Project context" value={projectId} onChange={event => { setProjectId(event.target.value); setSelected(null); load(event.target.value); }}>{projects.map(project => <option key={project.id} value={project.id}>{project.project_number} · {project.project_name}</option>)}</select></section>
    ...
    {selected && <section className="panel"><div className="panel-head"><div><span className="eyebrow">EXACT REVIEW DETAIL</span><h3>{selected.deliverable?.deliverable_ref || "Drawing review"} · {selected.revision?.revision_code}</h3><small>{selected.deliverable?.title} · {selected.discipline || "Discipline not set"}</small></div><span className="tag">{selected.lane}</span></div><div className="form-grid"><label>Review Category<select aria-label="Review Category" value={selected.review_category?.id || ""} onChange={event => setCategory(event.target.value)}><option value="">Select configured category</option>{categories.map(category => <option key={category.id} value={category.id}>{category.code} · {category.name}</option>)}</select></label><div><span className="eyebrow">REVISION POLICY</span><p className="muted">{selected.revision_policy.business_format}; DocumentVersion remains separate; exact revision is pinned.</p></div></div><div className="detail-actions">{selected.renditions.map(rendition => <a className="button-secondary" key={rendition.id} href={`/api/projects/${projectId}/engineering/drawing-review/${selected.id}/renditions/${rendition.id}/download`} target="_blank" rel="noreferrer">Open {rendition.rendition_kind} / PDF</a>)}</div><div className="boundary-grid"><div><b>{selected.findings.length}</b><small>Review findings</small></div><div><b>{selected.internal_comments.length}</b><small>Internal comments</small></div><div><b>{selected.authority_links.length}</b><small>Canonical external links</small></div><div><b>Human</b><small>Professional approval owner</small></div></div></section>}
  </div>;
}
