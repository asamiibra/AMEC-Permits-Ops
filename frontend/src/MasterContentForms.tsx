...
export type CanonicalForm = { id: string; ref: string; content_type: "FORM"; title: string; category?: { id: string; label: string } | null; description?: string; version?: number; version_status: string; updated?: string; versions?: Version[] };
...
function FormEditor({ item, categories, busy, onClose, onSave }: { item?: CanonicalForm; categories: Category[]; busy: boolean; onClose: () => void; onSave: (form: FormData) => Promise<void> }) {
  const [ref, setRef] = useState(item?.ref || ""); const [title, setTitle] = useState(item?.title || ""); const [category, setCategory] = useState(item?.category?.id || ""); const [description, setDescription] = useState(item?.description || ""); const [reason, setReason] = useState(""); const [file, setFile] = useState<File | null>(null);
  const submit = (event: React.FormEvent) => { event.preventDefault(); const form = new FormData(); if (!item) { form.append("content_type", "FORM"); form.append("ref", ref); } else { form.append("expected_current_version", String(item.version || 0)); form.append("change_reason", reason); } form.append("title", title); if (category) form.append("category_id", category); form.append("description", description); if (file) form.append("file", file); void onSave(form); };
  return <div className="dashboard-modal-backdrop"><form className="dashboard-modal" onSubmit={submit}><div className="dashboard-modal-head"><div><span className="eyebrow">{item ? "MODIFY · NEW VERSION" : "NEW MASTER FORM"}</span><h3>{item ? `${item.ref} · ${item.title}` : "New Form"}</h3></div><button type="button" className="modal-close" onClick={onClose}>×</button></div><label>Title / Name<input required value={title} onChange={event => setTitle(event.target.value)} /></label>{!item && <label>Reference<input required placeholder="F-001" value={ref} onChange={event => setRef(event.target.value)} /></label>}<label>Category<select value={category} onChange={event => setCategory(event.target.value)}><option value="">Select category</option>{categories.filter(row => row.allowed_content_types.includes("FORM")).map(row => <option value={row.id} key={row.id}>{row.label}</option>)}</select></label><label>Description<textarea value={description} onChange={event => setDescription(event.target.value)} /></label>{item && <label>Change reason<input required value={reason} onChange={event => setReason(event.target.value)} /></label>}<label>File{item && <small className="field-help">Leave empty to keep the current canonical file.</small>}<input type="file" accept=".pdf,.docx,.doc,.xlsx,.xls,.txt,.csv" required={!item} onChange={event => setFile(event.target.files?.[0] || null)} /></label><div className="dashboard-modal-foot"><button type="button" className="button-secondary" onClick={onClose}>Cancel</button><button className="button-primary" disabled={busy}>{busy ? "Verifying storage…" : item ? "Save new version" : "Save and verify"}</button></div></form></div>;
}
