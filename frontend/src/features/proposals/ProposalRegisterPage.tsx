...
export function ProposalRegisterPage({ role, onOpen, onNew }: { role: ProposalRole; onOpen: (id: string) => void; onNew: () => void }) {
  ...
  useEffect(() => {
    ...
    loadProposalRegister(role, query).then((result) => { if (active) { setData(result); setError(""); } }).catch((cause: unknown) => { if (active) setError(cause instanceof Error ? cause.message : "Proposal Register is unavailable."); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [filters, role]);
  const rows = data?.items || [];
  const activeLane = data?.filters?.lane || filters.lane;
  const activeLaneCount = data?.lane_counts?.[activeLane] ?? 0;
