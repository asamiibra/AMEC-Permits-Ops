...
export function ProposalRegisterPage({ role, onOpen, onNew }: { role: ProposalRole; onOpen: (id: string) => void; onNew: () => void }) {
  ...
  useEffect(() => {
    ...
    loadProposalRegister(role, query, controller.signal).then((result) => { if (active) { setData(result); setError(""); } }).catch((cause: unknown) => { if (active && !(cause instanceof DOMException && cause.name === "AbortError")) { setData(null); setError(cause instanceof Error ? cause.message : "Proposal Register is unavailable."); } }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; controller.abort(); };
  }, [filters, role, refreshKey]);
  const rows = data?.items || [];
  return <div className="proposal-feature-page">
    <header className="proposal-page-intro"><div><span className="eyebrow">PROPOSALS · REGISTER</span><h2>Proposal worklist</h2><p>Follow one Proposal identity from source intake through Engineering, commercial response, and Contract handoff.</p></div><div className="proposal-intro-actions"><span className="tag">{role === "RESPONSIBLE_ENGINEER" ? "ENGINEERING" : role === "COMMERCIAL_APPROVER" ? "BUSINESS DEVELOPMENT" : "OWNER"}</span><button type="button" className="button-secondary" onClick={() => { window.history.pushState({}, "", "/proposals/sources"); window.dispatchEvent(new PopStateEvent("popstate")); }}>Source Workspace</button><button type="button" className="button-primary" onClick={onNew} disabled={Boolean(data?.action_capabilities) && !data?.action_capabilities?.NEW_PROPOSAL?.available}><Icon name="plus" size={15} /> New Proposal</button></div></header>
    ...
    <section className="proposal-register panel" aria-label="Proposal Register"><div className="panel-head"><div><span className="eyebrow">CURRENT BUSINESS STATE</span><h3>{loading ? "Loading proposals…" : error ? "Proposal Register unavailable" : `${data?.count ?? 0} proposal${data?.count === 1 ? "" : "s"}`}</h3></div><small>Backend-driven lanes · current owner · next authorized action</small></div><div className="proposal-table-wrap"><table className="proposal-table"><thead><tr><th>Proposal</th><th>Client</th><th>Stage / owner</th><th>Commercial value</th><th>Next action</th><th>Last activity</th><th /></tr></thead><tbody>{loading && <tr><td colSpan={7}><div className="proposal-empty"><b>Loading Proposal work…</b><span>Reading the canonical Proposal register.</span></div></td></tr>}{!loading && !error && !rows.length && <tr><td colSpan={7}><div className="proposal-empty"><b>No Proposal records match this view.</b><span>Try another lane, client, stage, or reference.</span></div></td></tr>}{!loading && !error && rows.map((row) => <RegisterRow key={row.id} row={row} onOpen={onOpen} />)}</tbody></table></div><div className="proposal-mobile-list">{!loading && !error && rows.map((row) => <RegisterCard key={row.id} row={row} onOpen={onOpen} />)}</div></section>
