...
export function NewProposalPage({ role, onBack, onCreated }: { role: ProposalRole; onBack: () => void; onCreated: (id: string) => void }) {
  ...
  const chooseClient = (value: string) => {
    const match = clientState.items.find((item) => item.name.toLowerCase() === value.trim().toLowerCase());
    update("client", value);
