...
type Entry = { id: string; path: string; name: string; is_directory: boolean; size: number; modified_ns: number; content_type: string; logical_category?: CategoryKey; category_source?: string };
...
export function ProposalSourceWorkspace({ role = "OWNER_SPONSOR", onBack, onOpenEditor }: { role?: string; onBack?: () => void; onOpenEditor?: (proposalId: string, revisionId: string, projectNumber: number) => void }) {
  ...
  const loadTree = () => {
    ...
    setLoading(true); setPreview(""); setActiveFolder(""); setCategory("ALL"); setExcludedSources(new Set());
    api<{ entries?: Entry[] }>("/api/proposals/sources/2026/projects/" + selected + "/tree", { headers: roleHeaders(role) })
      .then((data) => { const next = data.entries || []; setEntries(next); setCategoryDrafts(Object.fromEntries(next.filter((entry) => !entry.is_directory).map((entry) => [entry.id, (entry.logical_category || classify(entry)) as CategoryKey]))); })
  ...
  const createProposal = async () => {
    ...
    try {
      ...
      const data = await api<{ proposal_id?: string; proposal_reference?: string; editor_ready?: boolean; editor_revision_id?: string; source_count?: number }>("/api/proposals/sources/2026/projects/" + selected + "/create-proposal", { method: "POST", headers: roleHeaders(role), body: JSON.stringify({ excluded_source_paths }) });
  ...
  return <section className="panel proposal-source-workspace" aria-label="Proposal source workspace">
    ...
    <div className="source-workspace-toolbar"><button className="button-secondary" onClick={() => void sync()}>Sync now</button><button className="button-primary" onClick={() => void createProposal()} disabled={!selected || hasDirtyCategories}>Create Proposal</button><span className="muted">Draft · Synology read-only · writes: 0{hasDirtyCategories ? " · Save category changes before creating" : ""}</span></div>
