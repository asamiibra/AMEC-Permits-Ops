...
export function ProposalSourceWorkspace({ role = "OWNER_SPONSOR", onBack, onOpenEditor }: { role?: string; onBack?: () => void; onOpenEditor?: (proposalId: string, revisionId: string) => void }) {
  ...
  const openFile = async (entry: Entry) => { try { const response = await fetch("/api/proposals/sources/2026/projects/" + selected + "/files/" + entry.id + "/content", { headers: roleHeaders(role) }); if (!response.ok) throw new Error(await response.text()); const blob = await response.blob(); const url = URL.createObjectURL(blob); if (entry.content_type.startsWith("image/") || entry.content_type === "application/pdf") setPreview(url); else { const opened = window.open(url, "_blank"); if (!opened) { const link = document.createElement("a"); link.href = url; link.target = "_blank"; link.rel = "noreferrer"; link.click(); } } } catch (cause) { setError(cause instanceof Error ? cause.message : "File could not be opened."); } };
