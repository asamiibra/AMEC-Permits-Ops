...
export function ProposalSourceWorkspace({ role = "OWNER_SPONSOR", onBack, onOpenEditor, onOpenProposal }: { role?: string; onBack?: () => void; onOpenEditor?: (proposalId: string, revisionId: string, projectNumber: number) => void; onOpenProposal?: (proposalId: string) => void }) {
  ...
  return <section className="panel proposal-source-workspace" aria-label="Proposal source workspace">
    ...
    {error && !showSnapshotIncomplete && <div className="error-state" role="alert"><BidiText>{error}</BidiText><button className="button-secondary" onClick={() => { setError(""); void loadTree(); }}>Retry</button></div>}
    {message && <div className="inline-message" role="status"><BidiText>{message}</BidiText></div>}
    ...
    {createdProposal && <div className="inline-message" role="status"><BidiText>Created Proposal {createdProposal}</BidiText></div>}
    <div className="source-workspace-projects" role="tablist" aria-label="Synced source projects">{projects.map((project) => <button key={project.number} role="tab" aria-selected={selected === project.number} className={selected === project.number ? "active" : ""} onClick={() => setSelected(project.number)}><BidiText as="b">{project.number} · {project.name.replace(/^\d+\s*-\s*/, "")}</BidiText><small>Draft · {project.file_count} files</small></button>)}</div>
