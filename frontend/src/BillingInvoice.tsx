...
type BillingItem = { invoice: any; revision: any; stage: string; receivable: any };
...
export function BillingInvoicePage() {
  ...
  return <section className="billing-page">
    ...
    <div className="billing-table" role="table" aria-label="Invoices"><div className="billing-row billing-head" role="row"><b>Invoice</b><b>Invoice Ref</b><b>Contract / Project context</b><b>Amount</b><b>Due Date</b><b>Open</b></div>{filtered.map((item) => <div className="billing-row" role="row" key={item.invoice.id}><div><b>{item.invoice.invoice_reference}</b><small>{human(item.stage)}</small></div><span>{item.invoice.invoice_ref_status === "ALLOCATED" ? item.invoice.invoice_reference : "Not allocated"}</span><span>{item.invoice.contract_id}<small>{item.invoice.project_id || "Project context pending"}</small></span><span>{item.revision?.payable_total ?? "—"} {item.revision?.currency || ""}</span><span>{item.revision?.due_date || "—"}</span><button className="text-button" onClick={() => open(item.invoice.id)}>Open →</button></div>)}{!filtered.length && <div className="billing-empty"><b>No invoices in this lane.</b><p>Invoice drafts appear only from an eligible Billing Milestone.</p></div>}</div>
...
function InvoiceWorkspace({ detail, onBack, onRefresh }: { detail: any; onBack: () => void; onRefresh: () => void }) {
  ...
  return <section className="billing-page"><button className="text-button" onClick={onBack}>← Billing / Invoice list</button><div className="billing-intro"><div><span className="eyebrow">INVOICE WORKSPACE</span><h2>{invoice.invoice_reference}</h2><p>{detail.contract?.contract_name || detail.contract?.contract_reference || "Contract"} · exact ContractRevision pinned</p></div><span className={`status status-${String(invoice.status).toLowerCase().replaceAll("_", "-")}`}>{human(invoice.status)}</span></div>
    <div className="billing-summary-grid"><div><small>Client</small><b>{invoice.client_account_id || "—"}</b></div><div><small>Project</small><b>{invoice.project_id || "—"}</b></div><div><small>Payable total</small><b>{revision?.payable_total ?? "—"} {revision?.currency || ""}</b></div><div><small>Due date</small><b>{revision?.due_date || "—"}</b></div></div>
    ...
    <div className="billing-safe-note"><b>Receivable</b><span>{human(detail.receivable?.state)} · outstanding {detail.receivable?.outstanding_amount ?? "—"}. Paid is derived from verified allocation and is not financial settlement.</span></div>
  </section>;
}
