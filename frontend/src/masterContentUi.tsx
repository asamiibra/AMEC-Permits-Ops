...
export function Drawer({ title, eyebrow, children, footer, onClose, wide = false }: { title: string; eyebrow: string; children: ReactNode; footer: ReactNode; onClose: () => void; wide?: boolean }) {
  const submitEditor = (event: React.MouseEvent<HTMLElement>) => { const target = event.target as HTMLButtonElement; if (target.tagName === "BUTTON" && target.type !== "button" && !target.disabled) document.querySelector<HTMLFormElement>("form.content-editor-form")?.requestSubmit(); };
