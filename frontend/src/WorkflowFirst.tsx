...
const businessStatus = (value: unknown) => String(value || "Not recorded").replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase());
const businessRole = (value: unknown) => {
  const role = String(value || "Assigned reviewer").toUpperCase();
  if (["SYSTEM_ADMIN", "OWNER", "OWNER_SPONSOR"].includes(role)) return "Owner";
  if (["COMMERCIAL_APPROVER", "BUSINESS_DEVELOPMENT"].includes(role)) return "Business Development";
  if (["RESPONSIBLE_ENGINEER", "ENGINEERING", "PERMIT_PREPARER"].includes(role)) return "Engineering";
  return String(value || "Assigned reviewer").replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
};
const sourceSystemLabel = (value: unknown) => ({ SYNOLOGY: "Synology", EXCEL: "Excel", MUNICIPALITY: "Municipality" } as Record<string, string>)[String(value || "").toUpperCase()] || businessStatus(value);
const stageLabel = (stage: WorkflowStage) => WORKFLOW_STAGES.find((item) => item.code === stage)?.label || stage;
...
export function PermitStageStepper({ currentStage, activeStage, onSelect }: { currentStage: WorkflowStage; activeStage: WorkflowStage; onSelect: (stage: WorkflowStage) => void }) {
  ...
  return <><div className="workflow-stepper" aria-label="Permit lifecycle">
    {WORKFLOW_STAGES.map((item, index) => <button key={item.code} className={`workflow-step ${item.code === activeStage ? "active" : ""} ${item.code === currentStage ? "current" : ""} ${index < current ? "complete" : ""}`} onClick={() => onSelect(item.code)} aria-current={item.code === currentStage ? "step" : undefined}>
      <span className="workflow-step-dot">{index < current ? <Icon name="check" size={14} /> : item.number}</span><span>{item.short}</span>{item.code === currentStage && <small>YOU ARE HERE</small>}
    </button>)}
  </div><StageContext currentStage={currentStage} activeStage={activeStage} />;</>;
}

function StageContext({ currentStage, activeStage }: { currentStage: WorkflowStage; activeStage: WorkflowStage }) {
  const currentLabel = WORKFLOW_STAGES.find((stage) => stage.code === currentStage)?.label || currentStage;
  const viewedLabel = WORKFLOW_STAGES.find((stage) => stage.code === activeStage)?.label || activeStage;
  return <div className="stage-context" aria-label="Stage context"><div><span>Current stage</span><b>{currentLabel}</b></div><div><span>Viewing</span><b>{viewedLabel}</b></div></div>;
}

function NextActionCard({ action, onOpen, loading, canAct = true }: { action: NextAction; onOpen: () => void; loading?: boolean; canAct?: boolean }) {
  return <section className={`next-action-card ${action.blocking ? "blocking" : ""}`} aria-label="Next action"><div><span className="eyebrow">NEXT ACTION</span><h3>{action.action_label}</h3><p>{action.reason}</p><small>Owner: <b>{businessRole(action.owner_role)}</b> · Stage: {stageLabel(action.stage)}{!canAct && " · Context only for this role"}</small></div><button className={canAct ? "button-primary" : "button-secondary"} onClick={onOpen} disabled={loading}>{loading ? "Confirming…" : canAct ? action.action_label : "View current stage"}</button></section>;
}
...
export function MyWorkPage({ projects, applications, openPermit, openAbout }: { projects: Project[]; applications: Application[]; openPermit: (projectId: string, stage?: WorkflowStage) => void; openAbout?: () => void }) {
  ...
  return <div className="workflow-page"><PageIntro kicker="MY WORK" title="Resume permit work" description="One prioritized worklist for assigned actions, reviews, blockers, authority changes, and delivery failures. Every card opens the exact permit stage." />{openAbout && <section className="about-entry-card"><div className="about-entry-mark"><Icon name="guide" size={20} /></div><div><span className="eyebrow">NEW TO PERMITOPS?</span><h3>See how PermitOps works</h3><p>See the workflow, what is included in the current MVP, and how permit information stays controlled and traceable.</p></div><button className="button-primary" onClick={openAbout}>Explore PermitOps <Icon name="arrow-up-right" size={14} /></button></section>}<UnifiedMyWorkPanel />
...
export function PermitWorkspacePage({ project, application, persona, activeStage, openStage, openLegacy, backToPermits }: { project: Project; application?: Application | null; persona?: Persona; activeStage: WorkflowStage; openStage: (stage: WorkflowStage) => void; openLegacy: (page: string, projectId?: string) => void; backToPermits: () => void }) {
  ...
  const refreshWorkspace = () => Promise.all([api<any>(`/api/projects/${project.id}${issueId ? `?issue=${encodeURIComponent(issueId)}` : ""}`), api<any>(`/api/findings?project_id=${project.id}`)]).then(([nextDetail, value]) => { if (!nextDetail?.workflow || !Array.isArray(value?.findings)) throw new Error("Workspace response is incomplete."); const selectedId = nextDetail.workflow?.sources?.application_id; if (selectedId && Array.isArray(nextDetail.applications)) nextDetail.applications = [...nextDetail.applications].sort((left: any, right: any) => Number(right.id === selectedId) - Number(left.id === selectedId)); setDetail(nextDetail); setFindings(value.findings); });
  useEffect(() => { refreshWorkspace().catch((error) => setCommandError(error instanceof Error ? error.message : "Workspace data unavailable.")); }, [project.id, issueId]);
  ...
  return <div className="workflow-page permit-workspace"><button className="back-button" onClick={backToPermits}><Icon name="arrow-left" size={14} /> Permit portfolio</button>{notificationContext && <section className="notification-context" aria-label="Opened from notification" tabIndex={-1} ref={contextRef}><span className="eyebrow">OPENED FROM NOTIFICATION</span><h3>{notificationContext.subject}</h3><small>{notificationContext.created_at ? new Date(notificationContext.created_at).toLocaleString([], { dateStyle: "medium", timeStyle: "short" }) : "—"}{notificationContext.historical ? " · Historical event" : ""}</small><p>{notificationContext.historical ? "This event is historical context. Review the current Permit state below." : "This destination was opened from a Notification; reading it does not change the Permit workflow state."}</p>{notificationContext.context_only && <span className="tag">Context only for {contextPersona === "OWNER" ? "Owner" : contextPersona === "BUSINESS_DEVELOPMENT" ? "Business Development" : "Engineering"}</span>}<a className="button-secondary" href={backHref}><Icon name="arrow-left" size={14} /> Back to Notifications</a>{readError && <div className="notification-read-warning" role="alert">Could not persist the read acknowledgement. The notification remains unread.</div>}</section>}<PermitIdentity project={project} application={detail?.applications?.[0] || application} /><PermitStageStepper currentStage={currentStage} activeStage={activeStage} onSelect={openStage} /><section className="stage-context-strip" aria-label="Stage context"><div><small>Current stage</small><b>{stageLabel(currentStage)}</b></div><div><small>Viewing</small><b>{stageLabel(activeStage)}</b></div></section><div className="breadcrumb"><span>My Work</span><b>›</b><span>{project.project_number}</span><b>›</b><strong>{stageLabel(activeStage)}</strong></div><NextActionCard action={action} canAct={canAct} loading={commandLoading} onOpen={action.action_code === "ESTABLISH_PROJECT_SOURCES" && canAct ? confirmProjectSources : () => openStage(action.stage)} />{commandMessage && <div className="inline-message" role="status">{commandMessage}</div>}{commandError && <div className="error-banner" role="alert">{commandError}</div>}
...
function SourcesStage({ project, detail, workflowStage }: { project: Project; detail: any; workflowStage: WorkflowStage }) {
  const links = detail?.links || []; const complete = workflowStage !== "PROJECT_AND_SOURCES" || Boolean(detail?.workflow?.confirmed_at); return <section className="panel stage-panel"><div className="stage-heading"><div><span className="eyebrow">STAGE 1 · PROJECT & SOURCES</span><h3>Establish the permit workspace</h3><p>Confirm project identity and the source-system links that drive later verification.</p></div><span className={`tag ${complete ? "status-approved" : ""}`}>{complete ? "COMPLETE" : links.length >= 3 ? "READY FOR CONFIRMATION" : "BLOCKED"}</span></div><div className="source-grid"><div><span className="eyebrow">PROJECT</span><dl><dt>Project number</dt><dd>{project.project_number}</dd><dt>Consultancy office</dt><dd>{detail?.office?.name_en || "—"}</dd><dt>Project type</dt><dd>{businessStatus(project.workstream || project.permit_type)}</dd><dt>Confirmation evidence</dt><dd>{detail?.workflow?.confirmed_at ? `${new Date(detail.workflow.confirmed_at).toLocaleString()} · Confirmed by ${businessRole(detail.workflow.confirmed_by)}` : "Pending human confirmation"}</dd></dl></div><div><span className="eyebrow">SOURCE SYSTEMS</span>{links.length ? links.map((link: any) => <div className="source-link" key={link.id}><span className="source-check"><Icon name="check" size={15} /></span><div><b>{sourceSystemLabel(link.system_type)}</b><small>{link.display_reference}</small></div><span className="tag">Linked to this project</span>{String(link.system_type).toUpperCase() === "MUNICIPALITY" && <span className="tag">Simulator</span>}</div>) : <Blocker text="Missing required source-system link" owner="Engineering" evidence="Project identity and source reference" />}</div></div></section>;
}
