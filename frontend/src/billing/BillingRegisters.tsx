...
function ReportsView() {
  ...
  return <RegisterFrame title="Reports" eyebrow="REPORTING" description="Bounded canonical report views. The reporting-year policy is resolved; YTD values fail closed until business-local timezone configuration exists."><Panel title="Reporting boundaries"><div className="billing-report-grid"><div><b>Invoice Report</b><p>{report?.invoice_report.length || 0} historical invoice record(s), including financially resolved history.</p></div><div><b>Open Receivables</b><p>{report?.open_receivables.length || 0} current unresolved exposure record(s).</p></div><div><b>Project Payment History</b><p>{report?.payment_history.length || 0} observed, verified, allocated, or reversed payment record(s).</p></div><div><b>Company / YTD</b><p>Policy: Calendar year · runtime: configuration required.</p></div></div><div className="billing-note"><b>System boundary</b><span>Source: {report?.source_of_truth || "canonical Billing read models"} · YTD policy: CALENDAR YEAR · runtime: {String(report?.ytd_status || "CONFIGURATION_REQUIRED").replaceAll("_", " ")}</span></div></Panel></RegisterFrame>;
}
...
function ControlsView() {
  ...
  return <RegisterFrame title="Controls" eyebrow="GOVERNANCE" description="Read-only controls over protected financial configuration. Approved policy truth is separate from runtime readiness."><Panel title="Financial control posture"><div className="billing-report-grid"><div><b>Account masters</b><p>{controls?.financial_account_masters.length || 0} configured master record(s).</p></div><div><b>Account versions</b><p>{controls?.financial_account_versions.length || 0} masked version record(s).</p></div><div><b>Invoice numbering</b><p>{humanize(controls?.invoice_numbering.status || "RESOLVED")}; production fail-closed: {controls?.invoice_numbering.production_fail_closed ? "yes" : "no"}.</p></div><div><b>AI authority</b><p>Canonical writes: 0 · protected actions: 0.</p></div></div><div className="billing-note"><b>Policy / runtime</b><span>Finance policies resolved: {controls?.resolved_owner_policies?.length || 0} · runtime configuration remains explicit where required.</span></div></Panel></RegisterFrame>;
}
