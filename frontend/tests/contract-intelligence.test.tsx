...
vi.mock("../src/contract/contractApi", () => ({
  getContractIntelligence: vi.fn().mockResolvedValue({
    ...
    execution_state: "COMMISSIONING_PENDING",
    eligibility_state: "COMMISSIONING_PENDING",
...
describe("Contract Intelligence", () => {
  it("shows an honest commissioning state instead of fake findings", async () => {
    render(<ContractIntelligence contractId="contract-1" />);
    expect(await screen.findByText(/COMMISSIONING_PENDING · shared model gateway and context compiler/)).toBeVisible();
    expect(screen.getByText("Governed runtime")).toBeVisible();
    expect(screen.getByText(/The 11 Contract skills use server-selected, authorized context/)).toBeVisible();
