...
test("Owner can navigate the business Administration surface and persist a setting", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Admin", level: 2 }).filter({ hasText: /^Admin$/ })).toBeVisible();
  await expect(page.getByRole("heading", { name: "System administration", level: 3 })).toBeVisible();
  await expect(page.getByText("Admin owns configuration, not business records", { exact: true })).toBeVisible();
