...
test("BD Proposal Forms-Driven v2 exposes the governed owner workspace", async ({ page, request }) => {
  ...
  await expect(page.getByText("Regulatory Scoping").first()).toBeVisible();
  await expect(page.getByText("Expected Client Inputs — Preliminary", { exact: true })).toBeVisible();
  await expect(page.getByText(/Area basis:/).first()).toBeVisible();
