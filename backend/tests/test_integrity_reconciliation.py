...
def test_safety_contract_has_no_machine_final_submit(client):
    ...
    permit_paths = " ".join(path for path in openapi["paths"].keys() if not path.startswith("/api/billing") and not path.startswith("/api/source18")).upper()
    ...
    assert "SIGNATURE" not in permit_paths
