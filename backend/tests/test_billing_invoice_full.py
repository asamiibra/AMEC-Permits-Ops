...
def test_billing_invoice_accept_issue_receivable_payment_and_separation(client):
    ...
    created = client.post(
        ...
        headers=headers("COMMERCIAL_APPROVER"),
        json={"milestone_ids": [first_id], "informational_lines": [{"description": "Full Contract Amount (display only)", "calculated_line_amount": "250000"}]},
    )
    assert created.status_code == 200, created.text
    ...
    assert client.post(f"/api/billing/invoice-revisions/{revision['id']}/approvals", headers=headers("COMMERCIAL_APPROVER"), json={"approval_type": "CLIENT_REFERENCE", "approval_reference": "SYN-APP-001", "status": "VERIFIED"}).status_code == 200
    ...
    assert receivable["state"] in {"DUE", "NOT_DUE"}
