...
def _fixture():
    ...
    with SessionLocal() as db:
        ...
        db.add(key); db.flush()
        assertion = SemanticValueAssertion(semantic_key_id=key.id, context_type="AuthorityCase", context_id=case.id, value_json="QAT-S3-001", value_type="STRING", source_type="AuthorityCase", source_id=case.id, source_version=version.id, verification_status="VERIFIED", authority="OWNER", asserted_by="step3-test")
        db.add(assertion)
    ...
    with SessionLocal() as db:
        ...
        db.commit()
        return {"case_id": case.id, "item_id": item.id, "version_id": version.id, "release_id": release.id, "key_id": key.id, "project_id": project.id}
...
def test_governed_prefill_is_pinned_cited_deterministic_and_non_mutating(client):
    ...
    assert result["fields"][0]["citations"][0]["document_version_id"] == fixture["version_id"]
...
def test_governed_prefill_fails_closed_for_access_stale_and_conflict(client):
    ...
    with SessionLocal() as db:
        db.add(SemanticValueAssertion(semantic_key_id=fixture["key_id"], context_type="AuthorityCase", context_id=fixture["case_id"], value_json="CONFLICTING", value_type="STRING", source_type="SyntheticSource", source_id=str(uuid4()), source_version=fixture["version_id"], verification_status="VERIFIED", authority="OWNER", asserted_by="step3-test"))
        db.commit()
...
def test_field_matrix_never_promotes_missing_or_unverified_values():
    ...
    missing = _field(rule, key, [], version_id="version-1", source_hash="a" * 64)
    unverified = _field(rule, key, [SemanticValueAssertion(id="unverified", semantic_key_id="key", context_type="AuthorityCase", context_id="case", value_json="raw", value_type="STRING", source_type="source", source_id="source", verification_status="OBSERVED", asserted_by="test")], version_id="version-1", source_hash="a" * 64)
