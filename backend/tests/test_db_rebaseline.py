...
BASELINE = ROOT / "backend/migrations/versions/baseline_r13_0059.py"
PHASE4 = ROOT / "backend/migrations/versions/phase4_corpus_app_integration.py"
...
def test_active_graph_and_legacy_archive_are_exact():
    assert sorted(path.name for path in (ROOT / "backend/migrations/versions").glob("*.py")) == [
        "baseline_r13_0059.py", "phase4_corpus_app_integration.py"
    ]
...
def test_baseline_is_explicit_and_fail_closed():
    ...
    assert 'revision = "baseline_r13_0059"' in source
    assert "down_revision = None" in source
    assert "Canonical root baseline downgrade is intentionally unsupported" in source
    for marker in (
        "backend.app.models",
...
def test_main_py_uses_only_dynamic_migration_readiness():
    ...
    assert (ROOT / "backend/app/db.py").read_bytes() == subprocess.check_output(
        ["git", "show", f"{SOURCE_SHA}:backend/app/db.py"], cwd=ROOT
    )
...
def _predeploy_manifest(head: str) -> dict:
    return {
        ...
        "database": {"engine": "postgresql", "major": 16, "migration_head": head},
...
def test_release_contract_accepts_new_head_and_rejects_old_head():
    verify_release_manifest(_predeploy_manifest("baseline_r13_0059"), SOURCE_SHA)
...
def test_old_0059_database_head_fails_closed(monkeypatch):
    monkeypatch.setattr(database, "repository_migration_head", lambda: "baseline_r13_0059")
    monkeypatch.setattr(database, "database_migration_heads", lambda: ("0059_entra_user_identity",))
    with pytest.raises(RuntimeError, match="baseline_r13_0059"):
...
def test_root_downgrade_is_not_destructive():
    ...
    with pytest.raises(RuntimeError, match="Canonical root baseline downgrade"):
        module.downgrade()
