...
def _ensure_templates(client):
    for ref, canonical_ref, title, usage in (("F-0003", "BD-PROP-001", "Synthetic BD v2 Proposal Template", "PROPOSAL_TEMPLATE"), ("F-0004", "BD-CHK-001", "Synthetic BD v2 Proposal Checklist", "PROPOSAL_CHECKLIST")):
        ...
        if not item:
            response = client.post("/api/master-content", data={"content_type": "FORM", "ref": canonical_ref, "title": title, "description": title, "used_in": '["BD"]'}, files={"file": (f"{canonical_ref}.txt", b"synthetic BD v2 content", "text/plain")}, headers=OWNER)
