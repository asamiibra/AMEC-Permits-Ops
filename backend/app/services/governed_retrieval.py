...
def _master(db: Session, item: MasterContentItem, version: DocumentVersion, access: RetrievalAccessContext, query: RetrievalQuery, *, profile=None, provenance=None, bindings=None, prefetched=False):
    ...
    if not prefetched:
        ...
        bindings = list(db.scalars(select(MasterContentModuleBinding).where(MasterContentModuleBinding.master_content_id == item.id, MasterContentModuleBinding.active == true())))
...
def governed_retrieve(db: Session, query: RetrievalQuery, access: RetrievalAccessContext) -> tuple[GovernedRetrievalResult, ...]:
    ...
    if query.master_content_id or (not query.document_version_id and not query.definition_entry_id):
        ...
        for b in (db.scalars(select(MasterContentModuleBinding).where(MasterContentModuleBinding.master_content_id.in_(item_ids), MasterContentModuleBinding.active == true())).all() if item_ids else []): bindings[b.master_content_id].append(b)
        for item in items:
