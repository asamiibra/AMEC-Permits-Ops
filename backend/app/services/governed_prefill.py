...
NON_AI_WRITERS = {"AUTHORITY_ONLY", "HUMAN_SIGNER", "EXTERNAL_PARTY"}
PROTECTED_FIELD_STATES = {"AUTHORITY_ONLY", "PROTECTED", "LOCKED", "DOMAIN_DERIVED"}
...
def _writer_policy_entry(profile: FormAutomationProfile, rule: FormMappingRule) -> dict[str, Any]:
    policy = profile.writer_policy_json or {}
    entries = [policy.get(rule.logical_field_key), policy.get(rule.target_key)]
    for entry in entries:
        if isinstance(entry, str):
            return {"writer": entry.upper()}
        if isinstance(entry, dict):
            return {str(key): value for key, value in entry.items()}
    return {}


def _field_write_state(profile: FormAutomationProfile, rule: FormMappingRule, draft: FormInstance | None) -> dict[str, Any]:
    policy = _writer_policy_entry(profile, rule)
    target_writer = str(policy.get("writer") or rule.target_writer or "SYSTEM").upper()
    values = draft.resolved_values if draft else {}
    metadata = (draft.field_write_metadata_json or {}).get(rule.logical_field_key, {}) if draft else {}
    has_value = rule.logical_field_key in values and values.get(rule.logical_field_key) not in (None, "")
    existing_state = str(metadata.get("writer_classification") or ("USER_ENTERED" if has_value else "EMPTY")).upper()
    if target_writer in NON_AI_WRITERS:
        return {"writer_classification": target_writer, "write_eligibility": "REJECTED", "write_reason": f"Target writer {target_writer} is outside governed human draft apply."}
    if policy.get("locked") is True or str(policy.get("state", "")).upper() == "LOCKED":
        return {"writer_classification": "LOCKED", "write_eligibility": "REJECTED", "write_reason": "Locked fields cannot be overwritten by governed prefill."}
    if policy.get("protected") is True or str(policy.get("state", "")).upper() in {"PROTECTED", "AUTHORITY_ONLY"}:
        return {"writer_classification": "PROTECTED", "write_eligibility": "REJECTED", "write_reason": "Protected fields require their existing domain writer."}
    if existing_state in PROTECTED_FIELD_STATES:
        return {"writer_classification": existing_state, "write_eligibility": "REJECTED", "write_reason": f"Existing {existing_state} value is protected from governed prefill."}
    if has_value and existing_state == "USER_ENTERED":
        return {"writer_classification": existing_state, "write_eligibility": "REJECTED", "write_reason": "Existing user-entered value requires an explicit domain edit."}
    if has_value and existing_state == "AI_ASSISTED" and policy.get("replaceable") is not True:
        return {"writer_classification": existing_state, "write_eligibility": "REJECTED", "write_reason": "Prior assisted value is not replaceable under the current writer policy."}
    return {"writer_classification": existing_state if has_value else "EMPTY", "write_eligibility": "READY", "write_reason": None}


def _decorate_field(field: dict[str, Any], profile: FormAutomationProfile, rule: FormMappingRule, draft: FormInstance | None) -> None:
    state = _field_write_state(profile, rule, draft)
    field.update(state)
    if state["write_eligibility"] != "READY" and field["proposal_status"] == "READY":
        field.update(proposal_status="REJECTED", authority_state="WRITER_POLICY", warning=state["write_reason"])
...
def preview_prefill(
    ...
    *,
    ...
    if role_value not in {"SYSTEM_ADMIN", "OWNER_SPONSOR", "PERMIT_PREPARER"}:
        raise ValueError("PREFILL_ROLE_NOT_ALLOWED")
    if purpose.strip().upper() != "FORM_PREPARATION":
    ...
    target_version = db.get(DocumentVersion, current_version_id) if current_version_id else None
    if not target_version or target_version.document_id != item.document_id or target_version.approval_state in {DocumentApprovalState.WORKING, DocumentApprovalState.SUPERSEDED} or target_version.document.current_version_id != target_version.id:
        raise ValueError("PREFILL_TARGET_VERSION_NOT_ELIGIBLE")
    ...
    for rule in rules:
        ...
        _decorate_field(field, profile, rule, draft)
        fields.append(field)
    ...
    material_fields = [
        {
            ...
            # Draft writer state is revalidated separately at Apply time. The
            # fingerprint captures the evidence proposal, not an unversioned
            # legacy human edit that should instead produce a field conflict.
            "proposal_status": "READY" if field["proposed_value"] is not None and field["provenance"] and field["authority_state"] in {"VERIFIED", "WRITER_POLICY"} else field["proposal_status"],
            "authority_state": "VERIFIED" if field["proposed_value"] is not None and field["provenance"] and field["authority_state"] in {"VERIFIED", "WRITER_POLICY"} else field["authority_state"],
    ...
    preview_fingerprint = _fingerprint(
        {
            ...
            "target_document_version_hash": target_version.sha256,
            ...
            "case_id": case_id,
            "purpose": purpose,
            "writer_policy": profile.writer_policy_json or {},
...
def apply_governed_prefill_to_draft(
    ...
    *,
    ...
    if not profile or profile.source_document_version_id != instance.source_document_version_id:
        raise ValueError("TARGET_FORM_CHANGED")
    target_version = db.get(DocumentVersion, instance.source_document_version_id)
    target_document = db.get(Document, target_version.document_id) if target_version else None
    if not target_version or not target_document or target_document.id != item.document_id or target_document.current_version_id != target_version.id or target_version.approval_state in {DocumentApprovalState.WORKING, DocumentApprovalState.SUPERSEDED}:
        raise ValueError("TARGET_DOCUMENT_VERSION_NOT_ELIGIBLE")
    if (
        not release
    ...
    if current_preview.get("preview_status") != "READY" and selected_field_keys is None:
        edited = [field["logical_field_key"] for field in current_preview["fields"] if field.get("writer_classification") in {"USER_ENTERED", "AI_ASSISTED", "DOMAIN_DERIVED"}]
        if edited:
            raise ValueError("HUMAN_EDIT_CONFLICT:" + ",".join(sorted(edited)))
    ...
    invalid_fields = [field for field in selected_fields if field["proposal_status"] != "READY" or field["authority_state"] != "VERIFIED" or field.get("write_eligibility") != "READY" or field["proposed_value"] is None or not field["provenance"]]
    if invalid_fields:
        edited = [field["logical_field_key"] for field in invalid_fields if field.get("writer_classification") in {"USER_ENTERED", "AI_ASSISTED", "DOMAIN_DERIVED"}]
        if edited:
            raise ValueError("HUMAN_EDIT_CONFLICT:" + ",".join(sorted(edited)))
        reasons = [f"{field['logical_field_key']}:{field.get('write_reason') or field.get('warning') or field['proposal_status']}" for field in invalid_fields]
        raise ValueError("FIELD_SELECTION_REJECTED:" + ";".join(reasons))
    ...
    rules_by_key = {rule.logical_field_key: rule for rule in db.scalars(select(FormMappingRule).where(FormMappingRule.mapping_release_id == release.id)).all()}
    conflicts: list[str] = []
    changed: list[str] = []
    for field in selected_fields:
        ...
        writer_state = _field_write_state(profile, rules_by_key[logical_key], instance)
        if writer_state["write_eligibility"] != "READY":
            conflicts.append(f"{logical_key}:{writer_state['write_reason']}")
        elif has_existing and _value_key(current) != _value_key(proposed):
            conflicts.append(f"{logical_key}:Existing value differs from governed proposal.")
        elif not has_existing:
            values[logical_key] = proposed
            write_metadata[logical_key] = {
                "mode": "GOVERNED_HUMAN_APPLY",
                "writer_classification": "AI_ASSISTED",
