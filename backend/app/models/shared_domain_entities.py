...
class AuthorityCase(Base, TimestampMixin):
    ...
    transaction_type: Mapped[str | None] = mapped_column(String(100), index=True)
    processing_mode: Mapped[str] = mapped_column(String(30), nullable=False, default="UNKNOWN")
    project_required: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    currentness_control_implemented: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    current_authority_policy_verified: Mapped[str] = mapped_column(String(30), nullable=False, default="UNKNOWN")
    current_official_form_verified: Mapped[str] = mapped_column(String(30), nullable=False, default="UNKNOWN")
    live_action_eligibility: Mapped[str] = mapped_column(String(40), nullable=False, default="BLOCKED_UNKNOWN")
    g5_blocking_currentness_gap: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    official_form_version_id: Mapped[str | None] = mapped_column(ForeignKey("document_versions.id"), index=True)
    official_form_publisher: Mapped[str | None] = mapped_column(String(240))
    official_form_number: Mapped[str | None] = mapped_column(String(120))
    official_form_revision: Mapped[str | None] = mapped_column(String(80))
    official_form_retrieved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    field_authority_schema_json: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False)
    packaging_requirements_json: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False)
    case_owner: Mapped[str | None] = mapped_column(String(200))
    task_executor: Mapped[str | None] = mapped_column(String(200))
    required_signer: Mapped[str | None] = mapped_column(String(200))
    internal_reviewer: Mapped[str | None] = mapped_column(String(200))
    first_blocker: Mapped[str | None] = mapped_column(String(240))
    next_accountable_action: Mapped[str | None] = mapped_column(String(500))
    opened_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    closed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_by: Mapped[str] = mapped_column(String(200), nullable=False)
