...
from sqlalchemy import JSON, Boolean, Date, DateTime, Enum as SAEnum, ForeignKey, Integer, String, Text
...
class ConsultancyOffice(Base, TimestampMixin):
    ...
    projects: Mapped[list["Project"]] = relationship(back_populates="office")
...
class User(Base, TimestampMixin):
    ...
    office: Mapped[ConsultancyOffice] = relationship(back_populates="users")


class Project(Base, TimestampMixin):
    __tablename__ = "projects"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    project_number: Mapped[str] = mapped_column(String(50), unique=True, nullable=False)
...
class ExternalSystemLink(Base):
    ...
    system_type: Mapped[SystemType] = mapped_column(SAEnum(SystemType), nullable=False)
    external_reference: Mapped[str] = mapped_column(String(300), nullable=False)
    display_reference: Mapped[str] = mapped_column(String(300), nullable=False)
    active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
...
class PermitApplication(Base, TimestampMixin):
    ...
    municipality: Mapped[str] = mapped_column(String(100), nullable=False)
    permit_type: Mapped[str] = mapped_column(String(100), nullable=False)
    external_request_number: Mapped[str] = mapped_column(String(100), unique=True, nullable=False)
...
class MinistryInquiry(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    question_code: Mapped[str] = mapped_column(String(100), unique=True, nullable=False)
    question: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[InquiryStatus] = mapped_column(SAEnum(InquiryStatus), nullable=False)
    client_owner: Mapped[str | None] = mapped_column(String(200))
...
class RaidItem(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    type: Mapped[RaidType] = mapped_column(SAEnum(RaidType), nullable=False)
    ...
    phase0_close_impact: Mapped[str] = mapped_column(String(30), default="NONE", nullable=False)
