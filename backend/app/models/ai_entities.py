...
class AIExecutionLedger(Base):
    ...
    reservation_owner_token: Mapped[str | None] = mapped_column(String(64), index=True)
    reservation_generation: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    reserved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    reservation_lease_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    reservation_reclaimed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
