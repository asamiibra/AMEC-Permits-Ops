...
class Contract(Base, TimestampMixin):
    ...
    agreement_type: Mapped[str] = mapped_column(String(80), default="AMEC_PROFESSIONAL_SERVICES", nullable=False, index=True)
...
class InvoiceRevision(Base):
    ...
    billing_plan_revision_id: Mapped[str | None] = mapped_column(ForeignKey("billing_plan_revisions.id"), index=True)
    ...
    due_date_basis: Mapped[str | None] = mapped_column(String(100))
    due_date_offset_days: Mapped[int | None] = mapped_column(Integer)
    due_date_fixed_date: Mapped[date | None] = mapped_column(Date)
    due_date_status: Mapped[str] = mapped_column(String(40), default="NOT_CONFIGURED", nullable=False)
    ...
    currency: Mapped[str | None] = mapped_column(String(20))
    gross_charge_total: Mapped[float | None] = mapped_column(Numeric(18, 2))
    adjustment_total: Mapped[float | None] = mapped_column(Numeric(18, 2), default=0)
    payable_total: Mapped[float | None] = mapped_column(Numeric(18, 2))
