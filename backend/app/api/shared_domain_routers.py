...
def create_form_instance(payload: dict[str, Any] = Body(default={}), request: Request = None, db: Session = Depends(get_db), role: Role = Depends(current_user_role)):
    ...
    requested_version_id = payload.get("source_document_version_id", profile.source_document_version_id)
    if requested_version_id != profile.source_document_version_id:
        raise HTTPException(409, {"code": "FORM_INSTANCE_SOURCE_VERSION_MISMATCH"})
