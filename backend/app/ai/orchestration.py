...
def execute_technical_methodology(db: Session, principal: AuthenticatedPrincipal, *, settings: Settings, project_id: str, client_request_id: str, correlation_id: str, provider: AIProvider | None = None) -> dict[str, object]:
    ...
    if settings.ai_provider_mode == "FOUNDRY_PROJECT_INSTANT_SYNTHETIC":
        response["model"].update({"access_mode": result.access_mode or "INSTANT", "region": settings.ai_foundry_project_region, "requested_model_name": result.requested_model_name or response["model"]["model"], "requested_model_version": result.requested_model_version or response["model"]["version"], "requested_model_id": result.requested_model_id or response["model"]["model"], "observed_response_model": result.observed_response_model or response["model"]["model"]})
    return response
