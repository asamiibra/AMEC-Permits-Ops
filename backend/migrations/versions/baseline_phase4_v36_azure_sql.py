...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    bind.execute(sa.text("INSERT INTO master_content_reference_sequences (active, content_type, created_at, current_value, id, padding, prefix, scope, updated_at) SELECT :value_0, CAST(:value_1 AS VARCHAR(40)), CURRENT_TIMESTAMP, :value_3, :value_4, :value_5, :value_6, CAST(:value_7 AS VARCHAR(80)), CURRENT_TIMESTAMP WHERE NOT EXISTS (SELECT 1 FROM master_content_reference_sequences WHERE content_type = CAST(:value_1 AS VARCHAR(40)) AND scope = CAST(:value_7 AS VARCHAR(80)))"),
