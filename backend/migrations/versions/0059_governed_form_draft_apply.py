...
def upgrade() -> None:
    ...
    for name, column_type, nullable, default in FORM_INSTANCE_COLUMNS:
        if name not in columns:
            op.add_column("form_instances", sa.Column(name, column_type, nullable=nullable, server_default=default))
    ...
    if "ix_form_instance_apply_commands_project_id" not in indexes:
        op.create_index("ix_form_instance_apply_commands_project_id", "form_instance_apply_commands", ["project_id"], unique=False)


def downgrade() -> None:
    bind = op.get_bind()
    indexes = {index["name"] for index in sa.inspect(bind).get_indexes("form_instance_apply_commands")}
    for name in ("ix_form_instance_apply_commands_project_id", "ix_form_instance_apply_form_instance"):
        if name in indexes:
